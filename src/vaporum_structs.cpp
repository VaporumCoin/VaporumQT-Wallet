 ...
 */
void vaporum_state::AddCheckpoint(const notarized_checkpoint &in)
{
 ...
 */
int32_t vaporum_state::NotarizedData(int32_t nHeight,uint256 *notarized_hashp,uint256 *notarized_desttxidp) const
{
    bool found = false;
 ...
 * @returns the notarized height
 */
int32_t vaporum_state::NotarizedHeight(int32_t *prevMoMheightp,uint256 *hashp,uint256 *txidp)
{
    CBlockIndex *pindex;
    if ( (pindex= vaporum_blockindex(last.notarized_hash)) == 0 || pindex->nHeight < 0 )
    {
        // found orphaned notarization, adjust the values in the vaporum_state object
 ...
 */
int32_t vaporum_state::PrevMoMHeight() const
{
    static uint256 zero;
    // shortcut
    if (last.MoM != zero)
 ...
 */
const notarized_checkpoint *vaporum_state::CheckpointAtHeight(int32_t height) const
{
...
void vaporum_state::clear_checkpoints() { NPOINTS.clear(); }
const uint256& vaporum_state::LastNotarizedHash() const { return last.notarized_hash; }
void vaporum_state::SetLastNotarizedHash(const uint256 &in) { last.notarized_hash = in; }
const uint256& vaporum_state::LastNotarizedDestTxId() const { return last.notarized_desttxid; }
void vaporum_state::SetLastNotarizedDestTxId(const uint256 &in) { last.notarized_desttxid = in; }
const uint256& vaporum_state::LastNotarizedMoM() const { return last.MoM; }
void vaporum_state::SetLastNotarizedMoM(const uint256 &in) { last.MoM = in; }
const int32_t& vaporum_state::LastNotarizedHeight() const { return last.notarized_height; }
void vaporum_state::SetLastNotarizedHeight(const int32_t in) { last.notarized_height = in; }
const int32_t& vaporum_state::LastNotarizedMoMDepth() const { return last.MoMdepth; }
void vaporum_state::SetLastNotarizedMoMDepth(const int32_t in) { last.MoMdepth =in; }
uint64_t vaporum_state::NumCheckpoints() const { return NPOINTS.size(); }
