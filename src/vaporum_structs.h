...
//extern std::mutex vaporum_mutex;  //todo remove

struct vaporum_event
{
    struct vaporum_event *related;
    uint16_t len;
    int32_t height;
...
{
public:
    event(vaporum_event_type t, int32_t height) : type(t), height(height) {}
    virtual ~event() = default;
    vaporum_event_type type;
    int32_t height;
};
...
{
    event_rewind() : event(vaporum_event_type::EVENT_REWIND, 0) {}
...
{
    event_notarized() : event(vaporum_event_type::EVENT_NOTARIZED, 0), notarizedheight(0), MoMdepth(0) {
        memset(this->dest, 0, sizeof(this->dest));
    }
    event_notarized(int32_t ht, const char* _dest) : event(vaporum_event_type::EVENT_NOTARIZED, ht), notarizedheight(0), MoMdepth(0) {
...
struct vaporum_ccdataMoM
{
    uint256 MoM;
    int32_t MoMdepth,notarized_height,height,txi;
};

struct vaporum_ccdata_entry { uint256 MoM; int32_t notarized_height,kmdheight,txi; char symbol[65]; };
struct vaporum_ccdatapair { int32_t notarized_height,MoMoMoffset; };

struct vaporum_ccdataMoMoM
{
    uint256 MoMoM;
    int32_t kmdstarti,kmdendi,MoMoMoffset,MoMoMdepth,numpairs,len;
    struct vaporum_ccdatapair *pairs;
};

struct vaporum_ccdata
{
    struct vaporum_ccdata *next,*prev;
    struct vaporum_ccdataMoM MoMdata;
    uint32_t CCid,len;
    char symbol[65];
};
...
public:
    ...
    uint64_t redeemed;
    uint64_t shorted;
    std::list<std::shared_ptr<vaporum::event>> events;
