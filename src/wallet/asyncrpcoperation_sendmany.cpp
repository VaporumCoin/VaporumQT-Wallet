...
bool AsyncRPCOperation_sendmany::main_impl() {
    ...
    if (isfromtaddr_) {
        ...
        if (isUsingBuilder_) {
            ...
            // for Vaporum, set lock time to accure interest, for other chains, set
            ...
            if (chainName.isKMD())
            {
                ...
                if ( !vaporum_hardfork_active((uint32_t)chainActive.Tip()->nTime) )
                    builder_.SetLockTime((uint32_t)time(NULL) - 60); // set lock time for Vaporum interest
                else
    ...
    while (!vpubNewProcessed) {
        ...
        while (numInputsNeeded++ < ZC_NUM_JS_INPUTS && zInputsDeque.size() > 0) {
            ...
            int wtxDepth = -1;
            {
                ...
                wtxHeight = vaporum_blockheight(wtx.hashBlock);
...
bool AsyncRPCOperation_sendmany::find_utxos(bool fAcceptCoinbase=false) {
    ...
    BOOST_FOREACH(const COutput& out, vecOutputs) {
        ...
        if( mindepth_ > 1 ) {
            ...
            int dpowconfs  = vaporum_dpowconfs(nHeight, out.nDepth);
