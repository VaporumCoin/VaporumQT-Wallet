...
UniValue convertpassphrase(const UniValue& params, bool fHelp, const CPubKey& mypk)
{
    if (fHelp || params.size() < 1 || params.size() > 1)
        throw runtime_error(
            ...
            "\"address\": \"vaporumaddress\",             (string) Address corresponding to your passphrase\n"
...
UniValue importprivkey(const UniValue& params, bool fHelp, const CPubKey& mypk)
{
    ...
    if (fHelp || params.size() < 1 || params.size() > 5)
        throw runtime_error(
            "importprivkey \"vaporumprivkey\" ( \"label\" rescan height secret_key)\n"
            "\nAdds a private key (as returned by dumpprivkey) to your wallet.\n"
            "\nArguments:\n"
            "1. \"vaporumprivkey\"   (string, required) The private key (see dumpprivkey)\n"
...
UniValue importaddress(const UniValue& params, bool fHelp, const CPubKey& mypk)
{
    ...
    if (IsValidDestination(dest)) {
        ...
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid Vaporum address or script");
    }
...
UniValue z_exportwallet(const UniValue& params, bool fHelp, const CPubKey& mypk)
{
    ...
    if (fHelp || params.size() != 1)
        throw runtime_error(
            ...
            "\nArguments:\n"
            "1. \"filename\"    (string, required) The filename, saved in folder set by vaporumd -exportdir option\n"
            "\nResult:\n"
...
UniValue dumpwallet_impl(const UniValue& params, bool fHelp, bool fDumpZKeys)
{
    ...
    if (exportdir.empty()) {
        throw JSONRPCError(RPC_WALLET_ERROR, "Cannot export wallet until the vaporumd -exportdir option has been set");
    }
    ...
    // produce output
    file << strprintf("# Wallet dump created by Vaporum %s (%s)\n", CLIENT_BUILD, CLIENT_DATE);
