...
bool DecrementNoteWitnesses(NoteDataMap& noteDataMap, int indexHeight, int64_t nWitnessCacheSize)
{
    ...
    assert(VAPORUM_REWIND != 0 || nWitnessCacheSize > 0 || WITNESS_CACHE_SIZE != _COINBASE_MATURITY+10);
    return true;
}
        ...
        if (mi != mapWallet.end())
        {
            ...
            if (txin.prevout.n < prev.vout.size())
                if (::IsMine(*this, prev.vout[txin.prevout.n].scriptPubKey) & filter)
                    return prev.vout[txin.prevout.n].nValue; // vaporum_interest?
        }
    }
    return 0;
}
...
std::vector<uint256> CWallet::ResendWalletTransactionsBefore(int64_t nTime)
{
    ...
    BOOST_FOREACH(PAIRTYPE(const uint256, CWalletTx)& item, mapWallet)
    {
        ...
        if ( (wtx.nLockTime >= LOCKTIME_THRESHOLD && wtx.nLockTime < now-VAPORUM_MAXMEMPOOLTIME) )
        {
                    ...
                    if ( !IS_MODE_EXCHANGEWALLET )
                    {
                        ...
                        if ( chainName.isKMD() && chainActive.Tip() != 0 && chainActive.Tip()->nHeight >= 60000 )
                        {
                            if ( pcoin->vout[i].nValue >= 10*COIN )
                            {
                                ...
                                {
                                    vaporum_accrued_interest(&txheight,&locktime,wtxid,i,0,pcoin->vout[i].nValue,(int32_t)tipindex->nHeight);
                                    interest = vaporum_interestnew(txheight,pcoin->vout[i].nValue,locktime,tipindex->nTime);
                                } 
                                else 
                                    interest = 0;
                                if ( interest != 0 )
                                ...
                                int& nChangePosRet, std::string& strFailReason, const CCoinControl* coinControl, bool sign)
{
    ...
    {
        if ( !vaporum_hardfork_active((uint32_t)chainActive.Tip()->nTime) )
