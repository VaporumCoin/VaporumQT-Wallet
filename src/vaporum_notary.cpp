...
static int didinit; // see vaporum_init
static uint8_t kmd_pubkeys[NUM_KMD_SEASONS][64][33]; // see vaporum_notaries
static int32_t hwmheight; // highest height ever passed to vaporum_notariesinit
static int32_t hadnotarization; // used in vaporum_dpowconfs
static bool didinit_NOTARIES[NUM_KMD_SEASONS]; // vaporum_notaries()
...
int32_t vaporum_isnotaryvout(char *coinaddr,uint32_t tiptime) // from ac_private chains only
{
    int32_t season = getacseason(tiptime);
    if ( NOTARY_ADDRESSES[season-1][0][0] == 0 )
    {
        uint8_t pubkeys[64][33];
        vaporum_notaries(pubkeys,0,tiptime);
    }
...
int32_t vaporum_notaries(uint8_t pubkeys[64][33],int32_t height,uint32_t timestamp)
{
    ...
    if ( chainName.isKMD() )   
        ...
        timestamp = vaporum_heightstamp(height); // derive the timestamp from the passed-in height

    // If this chain is not a staked chain, use the normal Vaporum logic to determine notaries. 
    ...
    std::lock_guard<std::mutex> lock(vaporum_mutex);
...
int32_t vaporum_electednotary(int32_t *numnotariesp,uint8_t *pubkey33,int32_t height,uint32_t timestamp)
{
    int32_t i,n; uint8_t pubkeys[64][33];
    n = vaporum_notaries(pubkeys,height,timestamp);
    *numnotariesp = n;
    for (i=0; i<n; i++)
    {
...
int32_t vaporum_ratify_threshold(int32_t height,uint64_t signedmask)
{
...
int32_t vaporum_chosennotary(int32_t *notaryidp,int32_t height,uint8_t *pubkey33,uint32_t timestamp)
{
...
int32_t vaporum_MoMdata(int32_t *notarized_htp,uint256 *MoMp,uint256 *kmdtxidp,int32_t height,uint256 *MoMoMp,int32_t *MoMoMoffsetp,int32_t *MoMoMdepthp,int32_t *kmdstartip,int32_t *kmdendip)
{
    const notarized_checkpoint *np = vaporum_npptr(height);
    if ( np != nullptr )
    {
...
/***
 * Add a notarized checkpoint to the vaporum_state
 * @param[in] sp the vaporum_state to add to
 ...
 */
void vaporum_notarized_update(struct vaporum_state *sp,int32_t nHeight,int32_t notarized_height,
        uint256 notarized_hash,uint256 notarized_desttxid,uint256 MoM,int32_t MoMdepth)
{
    if ( notarized_height >= nHeight )
    {
        LogPrintf("vaporum_notarized_update REJECT notarized_height %d > %d nHeight\n",notarized_height,nHeight);
        return;
    }
 ...
 */
void vaporum_init(int32_t height)
{
    uint256 zero; 
    uint8_t pubkeys[64][33];
    ...
    if ( !didinit )
    {
        ...
        if ( height >= 0 )
        {
            ...
            vaporum_notarysinit(0,pubkeys,count);
        }
        didinit = true;
        vaporum_stateupdate(0,0,0,0,zero,0,0,0,0,0,0,0,0,zero,0);
    }
}

/****
 * A nasty hack to reset statics in this file.
 * Do not put this in vaporum_notary.h, as it should only be used for testing
 */
void vaporum_notaries_uninit()
{
    didinit = 0;
    hwmheight = 0;
    hadnotarization = 0;
