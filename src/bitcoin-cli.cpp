...
static int AppInitRPC(int argc, char* argv[])
{
    ...
    if (argc<2 || mapArgs.count("-?") || mapArgs.count("-h") || mapArgs.count("-help") || mapArgs.count("-version")) {
        std::string strUsage = _("Vaporum RPC client version") + " " + FormatFullVersion() + "\n" + PrivacyInfo();
        if (!mapArgs.count("-version")) {
            strUsage += "\n" + _("Usage:") + "\n" +
                  "  vaporum-cli [options] <command> [params]  " + _("Send command to Vaporum") + "\n" +
                  "  vaporum-cli [options] help                " + _("List commands") + "\n" +
                  "  vaporum-cli [options] help <command>      " + _("Get help for a command") + "\n";
