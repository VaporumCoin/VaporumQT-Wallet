...
bool vaporum_is_vSolutionsFixActive(); // didn't want to bring vaporum headers here, it's a special case to bypass bad code in Solver() and ExtractDestination() 
    ...
    if (IsCryptoConditionsEnabled()) {
        ...
        if (scriptPubKey.IsPayToCryptoCondition(&ccSubScript, vParams))
        {
            if (scriptPubKey.MayAcceptCryptoCondition())
            {
                ...
                if (!vaporum_is_vSolutionsFixActive()) 
                {
...
bool ExtractDestination(const CScript& _scriptPubKey, CTxDestination& addressRet, bool returnPubKey)
{
    ...
    {
        if (vSolutions.size() > 1 && !vaporum_is_vSolutionsFixActive()) // allow this temporarily before the HF; actually this is incorrect to use opdrop's pubkey as the address
        {
