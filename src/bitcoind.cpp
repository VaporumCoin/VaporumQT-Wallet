...
bool AppInit(int argc, char* argv[])
{
    ...
    // Parameters
    //
    // If Qt is used, parameters/vaporum.conf are parsed in qt/bitcoin.cpp's main()
    ParseParameters(argc, argv);

    // Process help and version before taking care about datadir
    if (mapArgs.count("-?") || mapArgs.count("-h") ||  mapArgs.count("-help") || mapArgs.count("-version"))
    {
        std::string strUsage = _("Vaporum Daemon") + " " + _("version") + " " + FormatFullVersion() + "\n" + PrivacyInfo();
        ...
        {
            strUsage += "\n" + _("Usage:") + "\n" +
                  "  vaporumd [options]                     " + _("Start Vaporum Daemon") + "\n";
    ...
    try
    {
        ...
        try
        {
            ...
            LogPrintf(
                (_("Before starting vaporumd, you need to create a configuration file:\n"
                   "%s\n"
                   "It can be completely empty! That indicates you are happy with the default\n"
                   "configuration of vaporumd. But requiring a configuration file to start ensures\n"
                   "that vaporumd won't accidentally compromise your privacy if there was a default\n"
                   "option you needed to change.\n"
                   "\n"
                   "You can look at the example configuration file for suggestions of default\n"
                   "options that you may want to change. It should be in one of these locations,\n"
                   "depending on how you installed Vaporum:\n") +
                 _("- Source code:  %s\n"
                   "- .deb package: %s\n")).c_str(),
                GetConfigFile().string().c_str(),
                "contrib/debian/examples/vaporum.conf",
                "/usr/share/doc/vaporum/examples/vaporum.conf");
            return false;
        ...
        for (int i = 1; i < argc; i++)
            if (!IsSwitchChar(argv[i][0]) && !boost::algorithm::istarts_with(argv[i], "vaporum:"))
                fCommandLine = true;

        if (fCommandLine)
        {
            LogPrintf( "Error: There is no RPC client functionality in vaporumd. Use the vaporum-cli utility instead.\n");
            exit(EXIT_FAILURE);
        }
        ...
        if (fDaemon)
        {
            LogPrintf("Vaporum %s server starting\n",chainName.symbol().c_str());
