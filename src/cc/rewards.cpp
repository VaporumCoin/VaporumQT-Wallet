...
uint64_t RewardsCalc(int64_t amount, uint256 txid, int64_t APR, int64_t minseconds, int64_t maxseconds, uint32_t timestamp)
{
    ...
    */
    if ( !vaporum_hardfork_active(timestamp) )
...
{
    CMutableTransaction firstmtx,mtx = CreateNewContextualCMutableTransaction(Params().GetConsensus(), vaporum_nextheight());
    ...
    if ( amount > txfee )
    {
        reward = RewardsCalc((int64_t)amount,mtx.vin[0].prevout.hash,(int64_t)APR,(int64_t)minseconds,(int64_t)maxseconds,vaporum_chainactive_timestamp());
