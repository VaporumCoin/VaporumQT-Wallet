...
*/
extern int32_t vaporum_get_current_height();
...
{
    CMutableTransaction mtx = CreateNewContextualCMutableTransaction(Params().GetConsensus(), vaporum_nextheight()); CTransaction tx;
    CPubKey mypk,oraclespk; struct CCcontract_info *cp,C; std::string name,desc,format; int32_t numvouts; uint256 hashBlock;

    if (GetLatestTimestamp(vaporum_currentheight())<PUBKEY_SPOOFING_FIX_ACTIVATION)
...
UniValue OracleRegister(const CPubKey& pk, int64_t txfee,uint256 oracletxid,int64_t datafee)
{
    ...
    {
        if (GetLatestTimestamp(vaporum_currentheight())>PUBKEY_SPOOFING_FIX_ACTIVATION && AddMyOraclesFunds(cp,mtx,mypk,oracletxid)!=CC_MARKER_VALUE)
        ...
        if (GetLatestTimestamp(vaporum_get_current_height())>PUBKEY_SPOOFING_FIX_ACTIVATION) mtx.vout.push_back(CTxOut(txfee,CScript() << ParseHex(HexStr(mypk)) << OP_CHECKSIG));
