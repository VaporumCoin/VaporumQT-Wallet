...
///
/// One other technical note is that vaporumd has the insight-explorer extensions built in 
...
#define CC_REQUIREMENTS_MSG (VAPORUM_NSPV_SUPERLITE?"to use CC contracts you need to nspv_login first\n":"to use CC contracts, you need to launch daemon with valid -pubkey= for an address in your wallet\n")
...
/// Returns my pubkey, that is set by -pubkey vaporumd parameter
...
/// By using -addressindex=1 of vaporumd daemon, it allows tracking of all the CC addresses.
///
...
/// 'My pubkey' is the -pubkey parameter of vaporumd.
/// @param mtx mutable transaction object
/// @param total amount of inputs to add. If total equals to 0 the function does not add inputs but returns amount of all available normal inputs in the wallet
/// @param maxinputs maximum number of inputs to add
...
/// Logging category is set by -debug=category vaporumd param.
