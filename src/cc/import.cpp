        ...
        const std::vector<CTxOut>& vouts)
{
    CMutableTransaction mtx = CreateNewContextualCMutableTransaction(
            Params().GetConsensus(), vaporum_nextheight());
            ...
            uint256 bindtxid,std::vector<CPubKey> publishers,std::vector<uint256> txids,uint256 burntxid,int32_t height,int32_t burnvout,std::string rawburntx,CPubKey destpub, int64_t amount)
{
    ...
    { 
        LOGSTREAM("importgateway", CCLOG_INFO, stream << "CheckGATEWAYimport invalid import gateway. On this chain only valid import gateway is " << VAPORUM_EARLYTXID.GetHex() << std::endl);
        return(-1);
    }
...
bool Eval::ImportCoin(const std::vector<uint8_t> params, const CTransaction &importTx, unsigned int nIn)
{
    ...
    if (targetCcid < VAPORUM_FIRSTFUNGIBLEID)
