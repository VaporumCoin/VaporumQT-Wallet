...
int32_t games_sendrawtransaction(char *rawtx)
{
    ...
    if ( (retstr= vaporum_issuemethod(USERPASS,(char *)"sendrawtransaction",params,GAMES_PORT)) != 0 )
    {
...
int32_t games_progress(struct games_state *rs,int32_t waitflag,uint64_t seed,gamesevent *keystrokes,int32_t num)
{
    ...
    if ( rs->guiflag != 0 && Gametxidstr[0] != 0 )
    {
        ...
        if ( (retstr= vaporum_issuemethod(USERPASS,(char *)"cclib",params,GAMES_PORT)) != 0 )
        {
            if ( fp != 0 )
            {
                fprintf(fp,"%s\n",params);
...
int32_t games_setplayerdata(struct games_state *rs,char *gametxidstr)
{
    ...
    if ( 0 )
    {
        ...
        sprintf(cmd,"./vaporum-cli -ac_name=%s cclib gameinfo 17 \\\"[%%22%s%%22]\\\" > %s",ASSETCHAINS_SYMBOL,gametxidstr,fname);
        if ( system(cmd) != 0 )
    ...
    else
    {
        ...
        filestr = vaporum_issuemethod(USERPASS,(char *)"cclib",params,GAMES_PORT);
    }
    if ( filestr != 0 )
    {
...
int main(int argc, char **argv)
{
    ...
    GAMES_PORT = vaporum_userpass(userpass,ASSETCHAINS_SYMBOL);
    if ( IPADDRESS[0] == 0 )
    ...
    else
    {
        ...
        if ( GAMES_PORT == 0 )
        {
            printf("you must copy %s.conf from ~/.vaporum/%s/%s.conf (or equivalent location) to current dir\n",ASSETCHAINS_SYMBOL,ASSETCHAINS_SYMBOL,ASSETCHAINS_SYMBOL);
            return(-1);
        }
