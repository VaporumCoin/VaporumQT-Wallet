...
int64_t CCduration(int32_t &numblocks,uint256 txid)
{
    ...
    else if ( (pindex= vaporum_getblockindex(hashBlock)) == 0 || (txtime= pindex->nTime) == 0 || (txheight= pindex->nHeight) <= 0 )
    {
...
bool vaporum_txnotarizedconfirmed(uint256 txid)
{
    ...
    if ( VAPORUM_NSPV_SUPERLITE )
    {
        ...
        {
            LogPrintf("vaporum_txnotarizedconfirmed cant find txid %s\n",txid.ToString().c_str());
            return(0);
        }
        else if (txheight<=0)
        {
            LogPrintf("vaporum_txnotarizedconfirmed no txheight.%d for txid %s\n",txheight,txid.ToString().c_str());
            return(0);
        }
        else if (txheight>currentheight)
        {
            LogPrintf("vaporum_txnotarizedconfirmed backwards heights for txid %s hts.(%d %d)\n",txid.ToString().c_str(),txheight,currentheight);
            return(0);
        }
    ...
    else
    {
        ...
        {
            LogPrintf("vaporum_txnotarizedconfirmed no hashBlock for txid %s\n",txid.ToString().c_str());
            return(0);
        }
        else if ( (pindex= vaporum_blockindex(hashBlock)) == 0 || (txheight= pindex->nHeight) <= 0 )
        {
            LogPrintf("vaporum_txnotarizedconfirmed no txheight.%d %p for txid %s\n",txheight,pindex,txid.ToString().c_str());
            return(0);
        }
    ...
    if ((sp= vaporum_stateptr(symbol,dest)) != 0 && (notarized=sp->LastNotarizedHeight()) > 0 && txheight > sp->LastNotarizedHeight())  notarized=0;            
#ifdef TESTMODE           
    notarized=0;
#endif //TESTMODE
