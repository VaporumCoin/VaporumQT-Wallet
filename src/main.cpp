...
// Copyright (c) 2015-2020 The Vaporum Platform developers
...
const string strMessageMagic = "Vaporum Signed Message:\n";
...
UniValue vaporum_snapshot(int top)
{
    LOCK(cs_main);
    int64_t total = -1;
...
bool vaporum_snapshot2(std::map <std::string, CAmount> &addressAmounts)
{
...
bool vaporum_dailysnapshot(int32_t height)
{
    int reorglimit = 100; 
    ...
    // Under heavy reorg attack, its possible `vaporum_notarized_height` can return a height that can't be found on chain sync.
    ...
    else 
    {
        ...
        notarized_height = vaporum_notarized_height(&prevMoMheight,&notarized_hash,&notarized_desttxid);
    ...
    if ( !vaporum_snapshot2(addressAmounts) )
        return false;
    ...
    for (int32_t n = height; n > undo_height; n--) 
    {
        ...
        if ( (pindex= vaporum_chainactive(n)) == 0 || vaporum_blockload(block, pindex) != 0 ) 
            return false;
...
bool IsFinalTx(const CTransaction &tx, int nBlockHeight, int64_t nBlockTime)
{
    ...
    BOOST_FOREACH(const CTxIn& txin, tx.vin)
    {
        if ( !vaporum_hardfork_active(nBlockTime) && txin.nSequence == 0xfffffffe &&
        ...
        else if ( vaporum_hardfork_active(nBlockTime) && txin.nSequence == 0xfffffffe &&
            (
                ((int64_t)tx.nLockTime >= LOCKTIME_THRESHOLD && (int64_t)tx.nLockTime <= nBlockTime) ||
                ((int64_t)tx.nLockTime <  LOCKTIME_THRESHOLD && (int64_t)tx.nLockTime <= nBlockHeight))
            )
...
bool ContextualCheckCoinbaseTransaction(int32_t slowflag,const CBlock *block,CBlockIndex * const previndex,const CTransaction& tx, const int nHeight,int32_t validateprices)
{
    ...
    if (((uint64_t)(tx.GetValueOut()) >= ASSETCHAINS_TIMELOCKGTE) || (vaporum_ac_block_subsidy(nHeight) >= ASSETCHAINS_TIMELOCKGTE))
    {
            ...
            if (tx.vout[1].scriptPubKey.GetOp2(it, op, &opretData))
            {
                if (opretData.size() > 0 && opretData.data()[0] == OPRETTYPE_TIMELOCK)
                {
                    ...
                    if (CScriptID(opretScript) == scriptHash &&
                        ...
                        vaporum_block_unlocktime(nHeight) == unlocktime)
                    {
                        return(true);
                    }
                }
                      ...
                      libzcash::ProofVerifier& verifier,int32_t txIndex, int32_t numTxs)
{
    if ( chainName.isKMD() )
    {
        ...
        if ( *(int32_t *)&array[0] == 0 )
            numbanned = vaporum_bannedset(&indallvouts,array,(int32_t)(sizeof(array)/sizeof(*array)));

        for (size_t j=0; j<tx.vin.size(); j++) // for every tx.vin
        {
            for (int32_t k=0; k<numbanned; k++) // go through the array of banned txids
            {
                if ( tx.vin[j].prevout.hash == array[k] && vaporum_checkvout(tx.vin[j].prevout.n,k,indallvouts) != 0 ) //(tx.vin[j].prevout.n == 1 || k >= indallvouts) )
                {
    ...
    uint256 merkleroot;
    if ( ASSETCHAINS_STAKED != 0 && vaporum_newStakerActive(0, tiptime) != 0 && tx.vout.size() == 2 && DecodeStakingOpRet(tx.vout[1].scriptPubKey, merkleroot) != 0 )
    {
...
bool CheckTransactionWithoutProofVerification(uint32_t tiptime,const CTransaction& tx, CValidationState &state)
{
    ...
    int32_t invalid_private_taddr=0,z_z=0,z_t=0,t_z=0,acpublic = vaporum_acpublic(tiptime), current_season = getacseason(tiptime);
    /**
     * Previously:
    ...
    BOOST_FOREACH(const CTxOut& txout, tx.vout)
    {
        ...
        if ( ASSETCHAINS_PRIVATE != 0 )
        {
            ...
            if (iscoinbase == 0 && txout.nValue > 0)
            {
                ...
                if ( vaporum_isnotaryvout(destaddr,tiptime) == 0 )
                {
...
bool AcceptToMemoryPool(CTxMemPool& pool, CValidationState &state, const CTransaction &tx, bool fLimitFree,bool* pfMissingInputs, bool fRejectAbsurdFee, int dosLevel)
{
    ...
    if ( chainName.isKMD() && chainActive.Tip() != nullptr
            && !vaporum_validate_interest(tx,chainActive.Tip()->nHeight+1, chainActive.Tip()->GetMedianTimePast() + 777))
    {
        return state.DoS(0, error("%s: vaporum_validate_interest failed txid.%s", __func__, tx.GetHash().ToString()), REJECT_INVALID, "vaporum-interest-invalid");
    }
...
bool ReadBlockFromDisk(int32_t height,CBlock& block, const CDiskBlockPos& pos,bool checkPOW)
{
    ...
    if ( 0 && checkPOW != 0 )
    {
        vaporum_block2pubkey33(pubkey33,(CBlock *)&block);
...
CAmount GetBlockSubsidy(int nHeight, const Consensus::Params& consensusParams)
{
    ...
    {
        return(vaporum_ac_block_subsidy(nHeight));
    }
    /*
     // Mining slow start
    ...
    bool CheckTxInputs(const CTransaction& tx, CValidationState& state, const CCoinsViewCache& inputs, int nSpendHeight, const Consensus::Params& consensusParams)
    {
        ...
        for (unsigned int i = 0; i < tx.vin.size(); i++)
        {
            ...
            if (coins->IsCoinBase()) {
                ...
                {
                    uint64_t unlockTime = vaporum_block_unlocktime(coins->nHeight);
            ...
            if ( chainName.isKMD() && nSpendHeight > 60000 )
            {
                if ( coins->vout[prevout.n].nValue >= 10*COIN )
                {
                    ...
                    if ( (interest= vaporum_accrued_interest(&txheight,&locktime,prevout.hash,prevout.n,0,coins->vout[prevout.n].nValue,(int32_t)nSpendHeight-1)) != 0 )
                    {
                        nValueIn += interest;
                    }
                }
...
bool DisconnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    bool fClean = true;
    //vaporum_disconnect(pindex,block); does nothing?
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck,bool fCheckPOW)
{
    ...
    if ( ASSETCHAINS_NOTARY_PAY[0] != 0 && pindex->nHeight > 10 )
    {
        ...
        int32_t notarisationTx = vaporum_connectblock(true,pindex,*(CBlock *)&block);  
        ...
        if ( notarisationTx == 1 )
        {
            ...
            notarypaycheque = vaporum_checknotarypay((CBlock *)&block,(int32_t)pindex->nHeight);
    ...
    */
    ...
    if ( ASSETCHAINS_STAKED != 0 && fCheckPOW && vaporum_checkPOW(blockReward+stakeTxValue-notarypaycheque,1,(CBlock *)&block,pindex->nHeight) < 0 ) 
        return state.DoS(100, error("ConnectBlock: ac_staked chain failed slow vaporum_checkPOW"),REJECT_INVALID, "failed-slow_checkPOW");
    ...
    if ( ASSETCHAINS_COMMISSION != 0 || ASSETCHAINS_FOUNDERS_REWARD != 0 ) //ASSETCHAINS_OVERRIDE_PUBKEY33[0] != 0 &&
    {
        ...
        if ( (checktoshis= vaporum_commission((CBlock *)&block,(int32_t)pindex->nHeight)) != 0 )
        {
    ...
    //FlushStateToDisk();
    vaporum_connectblock(false,pindex,*(CBlock *)&block);  // dPoW state update.
...
bool static DisconnectTip(CValidationState &state, bool fBare = false) {
    ...
    //if ( ASSETCHAINS_SYMBOL[0] != 0 || pindexDelete->nHeight > 1400000 )
    {
        ...
        notarizedht = vaporum_notarized_height(&prevMoMheight,&notarizedhash,&txid);
    ...
    if (!fBare) {
        ...
        for (int i = 0; i < block.vtx.size(); i++)
        {
            ...
            if (tx.IsCoinBase() || (i == block.vtx.size()-1 && vaporum_newStakerActive(0, pindexDelete->nTime) == 0 && vaporum_isPoS((CBlock *)&block,pindexDelete->nHeight,0) != 0) || !AcceptToMemoryPool(mempool, stateDummy, tx, false, NULL))
            {
    ...
    {
        CTransaction &tx = block.vtx[i];
        //if ((i == (block.vtx.size() - 1)) && ((ASSETCHAINS_STAKED != 0 && (vaporum_isPoS((CBlock *)&block) != 0))))
        if ( vaporum_newStakerActive(0, pindexDelete->nTime) == 0 && i == block.vtx.size()-1 && vaporum_isPoS((CBlock *)&block,pindexDelete->nHeight,0) != 0 )
        {
#ifdef ENABLE_WALLET
...
int32_t vaporum_activate_sapling(CBlockIndex *pindex)
{
    uint32_t blocktime,prevtime; CBlockIndex *prev; int32_t i,transition=0,height,prevht;
    int32_t activation = 0;
    if ( pindex == 0 )
    {
        LogPrintf("vaporum_activate_sapling null pindex\n");
        return(0);
    }
    ...
    if ( activation != 0 )
    {
        vaporum_setactivation(activation);
...
bool CheckBlockHeader(int32_t *futureblockp,int32_t height,CBlockIndex *pindex, const CBlockHeader& blockhdr, CValidationState& state, bool fCheckPOW)
{
    ...
    *futureblockp = 0;
    ...
    /*vaporum_index2pubkey33(pubkey33,pindex,height);
                ...
                bool fCheckPOW, bool fCheckMerkleRoot)
{
    ...
    if ( fCheckPOW )
    {
        ...
        if ( ASSETCHAINS_STAKED == 0 && vaporum_checkPOW(0,1,(CBlock *)&block,height) < 0 ) // checks Equihash
    ...
    {
        int32_t notaryid;
        int32_t special = vaporum_chosennotary(&notaryid,height,pubkey33,tiptime);
        if (notaryid > 0 || ( notaryid == 0 && height > nS5HardforkHeight ) ) {
            ...
            if (!vaporum_checkopret(pblockcopy, merkleroot)) {
                LogPrintf("failed or missing merkleroot expected.%s != merkleroot.%s\n", vaporum_makeopret(pblockcopy, false).ToString().c_str(), merkleroot.ToString().c_str());
    ...
    if ( ASSETCHAINS_CC != 0 ) // CC contracts might refer to transactions in the current block, from a CC spend within the same block and out of order
    {
        ...
        while ( 1 )
        {
            ...
            for (i=0; i<block.vtx.size(); i++)
            {
                ...
                if ( tx.IsCoinBase() || !tx.vjoinsplit.empty() || !tx.vShieldedSpend.empty() || (i == block.vtx.size()-1 && vaporum_isPoS((CBlock *)&block,height,0) != 0) )
                    continue;
                Tx = tx;
    ...
    if ( fCheckPOW && vaporum_check_deposit(height,block) < 0 )
    {
        //static uint32_t counter;
        //if ( counter++ < 100 && ASSETCHAINS_STAKED == 0 )
        //    LogPrintf("check deposit rejection\n");
        LogPrintf("CheckBlockHeader vaporum_check_deposit error");
        return(false);
    }
...
bool ContextualCheckBlockHeader(const CBlockHeader& block, CValidationState& state, CBlockIndex * const pindexPrev)
{
    ...
    if (fCheckpointsEnabled)
    {
        ...
        if ( nHeight != 0 )
        {
            ...
            if ( !vaporum_checkpoint(&notarized_height,nHeight,hash) )
            {
                CBlockIndex *heightblock = chainActive[nHeight];
                if ( heightblock != 0 && heightblock->GetBlockHash() == hash )
                    return true;
...
bool ContextualCheckBlock(int32_t slowflag,const CBlock& block, CValidationState& state, CBlockIndex * const pindexPrev)
{
    ...
    const int32_t txheight = nHeight == 0 ? vaporum_block2height((CBlock *)&block) : nHeight;
    ...
    for (uint32_t i = 0; i < block.vtx.size(); i++) {
        ...
        // Interest validation
        if (!vaporum_validate_interest(tx, txheight, cmptime))
        {
            LogPrintf("validate intrest failed for txnum.%i tx.%s\n", i, tx.ToString());
            return state.DoS(0, error("%s: vaporum_validate_interest failed", __func__), REJECT_INVALID, "vaporum-interest-invalid");
        }
...
bool AcceptBlockHeader(int32_t *futureblockp,const CBlockHeader& block, CValidationState& state, CBlockIndex** ppindex)
{
    ...
    if (miSelf != mapBlockIndex.end())
    {
        ...
        /*if ( pindex != 0 && hash == vaporum_requestedhash )
        {
            LogPrintf("AddToBlockIndex A vaporum_requestedhash %s\n",vaporum_requestedhash.ToString().c_str());
            memset(&vaporum_requestedhash,0,sizeof(vaporum_requestedhash));
            vaporum_requestedcount = 0;
        }*/
    ...
    /*if ( pindex != 0 && hash == vaporum_requestedhash )
    {
        LogPrintf("AddToBlockIndex vaporum_requestedhash %s\n",vaporum_requestedhash.ToString().c_str());
        memset(&vaporum_requestedhash,0,sizeof(vaporum_requestedhash));
        vaporum_requestedcount = 0;
    }*/
    return true;
...
CBlockIndex *vaporum_ensure(CBlock *pblock, uint256 hash)
{
...
CBlockIndex *oldvaporum_ensure(CBlock *pblock, uint256 hash)
{
    CBlockIndex *pindex=0,*previndex=0;
    if ( (pindex = vaporum_getblockindex(hash)) == 0 )
    {
        pindex = new CBlockIndex();
        if (!pindex)
            throw runtime_error("vaporum_ensure: new CBlockIndex failed");
    ...
    if ( miSelf == mapBlockIndex.end() )
    {
        LogPrintf("vaporum_ensure unexpected missing hash %s\n",hash.ToString().c_str());
        return(0);
    }
    if ( miSelf->second == 0 ) // create pindex so first Accept block doesnt fail
    {
        ...
        if ( pindex != 0 )
        {
            miSelf->second = pindex;
            LogPrintf("Block header %s is already known, but without pindex -> ensured %p\n",hash.ToString().c_str(),miSelf->second);
        } else LogPrintf("vaporum_ensure unexpected null pindex\n");
    }
    /*if ( hash != Params().GetConsensus().hashGenesisBlock )
        {
            miSelf = mapBlockIndex.find(pblock->hashPrevBlock);
            ...
            if ( (miSelf= mapBlockIndex.find(pblock->hashPrevBlock)) != mapBlockIndex.end() )
            {
                if ( miSelf->second == 0 ) // create pindex so first Accept block doesnt fail
                {
                    ...
                    if ( previndex != 0 )
                    {
                        ...
                        LogPrintf("autocreate previndex %s\n",pblock->hashPrevBlock.ToString().c_str());
                    } else LogPrintf("vaporum_ensure unexpected null previndex\n");
                }
            } else LogPrintf("vaporum_ensure unexpected null miprev\n");
        }
     }*/
    return(pindex);
}
...
bool ProcessNewBlock(bool from_miner,int32_t height,CValidationState &state, CNode* pfrom, CBlock* pblock, bool fForceProcessing, CDiskBlockPos *dbp)
{
    ...
    hash = pblock->GetHash();
    {
        ...
        checked = CheckBlock(&futureblock,height!=0?height:vaporum_block2height(pblock),0,*pblock, state, verifier,0);
        bool fRequested = MarkBlockAsReceived(hash);
        fRequested |= fForceProcessing;
        if ( checked != 0 && vaporum_checkPOW(0,0,pblock,height) < 0 ) //from_miner && ASSETCHAINS_STAKED == 0
        {
            checked = 0;
...
//void vaporum_pindex_init(CBlockIndex *pindex,int32_t height);
...
bool static LoadBlockIndexDB()
{
    ...
    BOOST_FOREACH(const PAIRTYPE(uint256, CBlockIndex*)& item, mapBlockIndex)
    {
        ...
        //vaporum_pindex_init(pindex,(int32_t)pindex->nHeight);
    }
    ...
    if ( chainName.isKMD() ) {
        ...
        int32_t longestchain = vaporum_longestchain();
        // TODO: vaporum_longestchain does not have the data it needs at the time LoadBlockIndexDB
    ...
    if ( (pindex= chainActive.Tip()) != 0 )
    {
        if ( ASSETCHAINS_SAPLING <= 0 )
        {
            ...
            vaporum_activate_sapling(pindex);
        }
    }
    return true;
}
        ...
        while (!blkdat.eof()) {
            ...
            try {
                ...
                if (mapBlockIndex.count(hash) == 0 || (mapBlockIndex[hash]->nStatus & BLOCK_HAVE_DATA) == 0) {
                    ...
                    if (state.IsError())
                        break;
                } else if (hash != chainparams.GetConsensus().hashGenesisBlock && vaporum_blockheight(hash) % 1000 == 0) {
                    LogPrintf("Block Import: already had block %s at height %d\n", hash.ToString(), vaporum_blockheight(hash));
                }
...
#include "vaporum_nSPV_defs.h"
#include "vaporum_nSPV.h"            // shared defines, structs, serdes, purge functions
#include "vaporum_nSPV_fullnode.h"   // nSPV fullnode handling of the getnSPV request messages
#include "vaporum_nSPV_superlite.h"  // nSPV superlite client, issuing requests and handling nSPV responses
#include "vaporum_nSPV_wallet.h"     // nSPV_send and support functions, really all the rest is to support this

void vaporum_netevent(std::vector<uint8_t> payload);
...
//LogPrintf("netmsg: %s\n", strCommand.c_str());
    ...
    else if (GetBoolArg("-nspv_msg", DEFAULT_NSPV_PROCESSING) && strCommand == "nSPV")
    {
        if ( VAPORUM_NSPV_SUPERLITE )
        {
            std::vector<uint8_t> payload;
            vRecv >> payload;
            vaporum_nSPVresp(pfrom,payload);
        }
        return(true);
    }
...
bool SendMessages(CNode* pto, bool fSendTrickle)
{
    const Consensus::Params& consensusParams = Params().GetConsensus();
    {
        ...
        /*CBlockIndex *pindex;
        if ( vaporum_requestedhash != zero && vaporum_requestedcount < 16 && (pindex= vaporum_getblockindex(vaporum_requestedhash)) != 0 )
        {
            LogPrint("net","vaporum_requestedhash.%d request %s to nodeid.%d\n",vaporum_requestedcount,vaporum_requestedhash.ToString().c_str(),pto->GetId());
            LogPrintf("vaporum_requestedhash.%d request %s to nodeid.%d\n",vaporum_requestedcount,vaporum_requestedhash.ToString().c_str(),pto->GetId());
            vGetData.push_back(CInv(MSG_BLOCK, vaporum_requestedhash));
            MarkBlockAsInFlight(pto->GetId(), vaporum_requestedhash, consensusParams, pindex);
            vaporum_requestedcount++;
            if ( vaporum_requestedcount > 16 )
            {
                memset(&vaporum_requestedhash,0,sizeof(vaporum_requestedhash));
                vaporum_requestedcount = 0;
            }
