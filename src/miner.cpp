...
int32_t vaporum_waituntilelegible(uint32_t blocktime, int32_t stakeHeight, uint32_t delay)
{
...
CBlockTemplate* CreateNewBlock(const CPubKey _pk,const CScript& _scriptPubKeyIn, int32_t gpucount, bool isStake)
{
    ...
    CBlockIndex* pindexPrev = 0;
    {
        ...
        if ( ASSETCHAINS_NOTARY_PAY[0] != 0 )
        {
            ...
            numSN = vaporum_notaries(notarypubkeys, nHeight, pblock->nTime);
        }
             ...
             mi != mempool.mapTx.end(); ++mi)
        {
            ...
            if (chainName.isKMD() && !vaporum_validate_interest(tx, nHeight, cmptime))
            {
                LogPrintf("%s: vaporum_validate_interest failure txid.%s nHeight.%d nTime.%u vs locktime.%u (cmptime.%lu)\n", __func__, tx.GetHash().ToString(), nHeight, (uint32_t)pblock->nTime, (uint32_t)tx.nLockTime, cmptime);
                continue;
            }
            ...
            if (tx.IsCoinImport())
            {
                ...
                if ( numSN != 0 && notarypubkeys[0][0] != 0 && vaporum_is_notarytx(tx) == 1 )
            ...
            if ( fNotarisation ) 
            {
                ...
                if ( tx.vout.size() == 2 && tx.vout[1].nValue == 0 )
                {
                    ...
                    if ( script[0] == OP_RETURN )
                    {
                        ...
                        int32_t notarizedheight = vaporum_getnotarizedheight(pblock->nTime, nHeight, script, scriptlen);
        ...
        if ( !chainName.isKMD() && isStake )
        {
            ...
            blocktime = GetTime();
            uint256 merkleroot = vaporum_calcmerkleroot(pblock, pindexPrev->GetBlockHash(), nHeight, true, scriptPubKeyIn);
            ...
            */
            ...
            siglen = vaporum_staked(txStaked, pblock->nBits, &blocktime, &txtime, &utxotxid, &utxovout, &utxovalue, utxosig, merkleroot);
            if ( vaporum_newStakerActive(nHeight, blocktime) != 0 )
                nFees += utxovalue;
            ...
            if ( vaporum_waituntilelegible(blocktime, stakeHeight, delay) == 0 )
                return(0);
        ...
        uint64_t commission;
        if ( nHeight > 1 && !chainName.isKMD() && (ASSETCHAINS_OVERRIDE_PUBKEY33[0] != 0 || ASSETCHAINS_SCRIPTPUB.size() > 1) && (ASSETCHAINS_COMMISSION != 0 || ASSETCHAINS_FOUNDERS_REWARD != 0)  && (commission= vaporum_commission((CBlock*)&pblocktemplate->block,(int32_t)nHeight)) != 0 )
        {
            uint8_t *ptr;
            txNew.vout.resize(2);
        ...
        else if ( (uint64_t)(txNew.vout[0].nValue) >= ASSETCHAINS_TIMELOCKGTE)
        {
            ...
            opretScript.AddCheckLockTimeVerify(vaporum_block_unlocktime(nHeight));
        ...
        else if ( fNotarisationBlock && ASSETCHAINS_NOTARY_PAY[0] != 0 && pblock->vtx[1].vout.size() == 2 && pblock->vtx[1].vout[1].nValue == 0 )
        {
            ...
            if ( script[0] == OP_RETURN )
            {
                uint64_t totalsats = vaporum_notarypay(txNew, NotarisationNotaries, pblock->nTime, nHeight, script, scriptlen);
                if ( totalsats == 0 )
                {
        ...
        if ( chainName.isKMD() && IS_VAPORUM_NOTARY && My_notaryid >= 0 )
        {
            ...
            if ( Mining_height > nDecemberHardforkHeight ) //December 2019 hardfork
                opret = vaporum_makeopret(pblock, true);
            else
                opret.clear();

            if (vaporum_notaryvin(txNotary, NOTARY_PUBKEY33, opret, pblock->nTime) > 0)
            {
                CAmount txfees = 5000;
...
void vaporum_sendmessage(int32_t minpeers,int32_t maxpeers,const char *message,std::vector<uint8_t> payload)
{
    int32_t numsent = 0;
    LOCK(cs_vNodes);
...
void vaporum_broadcast(CBlock *pblock,int32_t limit)
{
...
#endif // ENABLE_WALLET
{
    ...
    // Found a solution
    {
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
        {
            ...
            return error("VaporumMiner: generated block is stale");
        }
    }
    ...
    if (!ProcessNewBlock(1,chainActive.Tip()->nHeight+1,state, NULL, pblock, true, NULL))
        return error("VaporumMiner: ProcessNewBlock, block not accepted");

    TrackMinedBlock(pblock->GetHash());
    //vaporum_broadcast(pblock,16);
    return true;
}
...
#endif
{
    LogPrintf("VaporumMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("vaporum-miner");
    const CChainParams& chainparams = Params();
    ...
    while ( (ASSETCHAIN_INIT == 0 || VAPORUM_INITDONE == 0) )
    {
        ...
        if ( vaporum_baseid(chainName.symbol().c_str()) < 0 )
            break;
    }
    if ( chainName.isKMD() )
    {
        ...
        vaporum_chosennotary(&notaryid,newHeight,NOTARY_PUBKEY33,timePast);
    }
...
//                                                                       [&m_cs, &cancelSolver](const uint256& hashNewTip) mutable {
    ...
    try {
        ...
        while (true)
        {
            ...
            if ( ptr == 0 )
            {
                if ( 0 && !GetBoolArg("-gen",false))
                {
                    miningTimer.stop();
                    c.disconnect();
                    LogPrintf("VaporumMiner terminated\n");
                    return;
                }
            ...
            {
                if (GetArg("-mineraddress", "").empty()) {
                    LogPrintf("Error in VaporumMiner: Keypool ran out, please call keypoolrefill before restarting the mining thread\n");
                } else {
                    // Should never reach here, because -mineraddress validity is checked in init.cpp
                    LogPrintf("Error in VaporumMiner: Invalid -mineraddress\n");
                }
                return;
            }
            ...
            if ( (chainName.isKMD() && notaryid >= 0 && Mining_height > nDecemberHardforkHeight ) || (ASSETCHAINS_STAKED != 0 && vaporum_newStakerActive(Mining_height, pblock->nTime) != 0) ) //December 2019 hardfork
                nExtraNonce = 0;
            IncrementExtraNonce(pblock, pindexPrev, nExtraNonce);
            //LogPrintf("Running VaporumMiner.%s with %u transactions in block\n",solver.c_str(),(int32_t)pblock->vtx.size());
            LogPrintf("Running VaporumMiner.%s with %u transactions in block (%u bytes)\n",solver.c_str(),pblock->vtx.size(),::GetSerializeSize(*pblock,SER_NETWORK,PROTOCOL_VERSION));
            //
            // Search
            //
            ...
            if ( chainName.isKMD() && notaryid >= 0 )
            {
                ...
                if ( (Mining_height >= 235300 && Mining_height < 236000) || (Mining_height % VAPORUM_ELECTION_GAP) > 64 || (Mining_height % VAPORUM_ELECTION_GAP) == 0 || Mining_height > 1000000 )
                {
                    ...
                    if ( notaryid <= 3 || notaryid == 32 || (notaryid >= 43 && notaryid <= 45) || notaryid == 51 || notaryid == 52 || notaryid == 56 || notaryid == 57 )
                        dispflag = 1;
                    vaporum_eligiblenotary(pubkeys,mids,blocktimes,&nonzpkeys,Mining_height);
                    if ( nonzpkeys > 0 )
                    {
                        for (i=0; i<33; i++)
                            if( pubkeys[0][i] != 0 )
                ...
                (std::vector<unsigned char> soln) {
                    ...
                    else
                    {
                        if ( VAPORUM_MININGTHREADS == 0 ) // we are staking 
                        {
                            ...
                            if ( vaporum_waituntilelegible(B.nTime, Mining_height, ASSETCHAINS_STAKED_BLOCK_FUTURE_MAX) == 0 )
                                return(false);
                        }
        ...
        catch (const std::runtime_error &e)
        {
            miningTimer.stop();
            c.disconnect();
            LogPrintf("VaporumMiner runtime error: %s\n", e.what());
            return;
        }
        miningTimer.stop();
        c.disconnect();
