...
#include "vaporum_utils.h" // portable_mutex_lock
#include "vaporum_notary.h" // vaporum_npptr
...
struct vaporum_ccdata_entry *vaporum_allMoMs(int32_t *nump,uint256 *MoMoMp,int32_t kmdstarti,int32_t kmdendi)
{
    ...
    DL_FOREACH_SAFE(CC_data,ccdata,tmpptr)
    {
        if ( ccdata->MoMdata.height <= kmdendi && ccdata->MoMdata.height >= kmdstarti )
        {
            ...
            {
                max += 100;
                allMoMs = (struct vaporum_ccdata_entry *)realloc(allMoMs,max * sizeof(*allMoMs));
            }
...
int32_t vaporum_addpair(struct vaporum_ccdataMoMoM *mdata,int32_t notarized_height,int32_t offset,int32_t maxpairs)
{
    if ( maxpairs >= 0) {
        if ( mdata->numpairs >= maxpairs )
        {
            maxpairs += 100;
            mdata->pairs = (struct vaporum_ccdatapair *)realloc(mdata->pairs,sizeof(*mdata->pairs)*maxpairs);
            //LogPrintf("pairs reallocated to %p num.%d\n",mdata->pairs,mdata->numpairs);
        }
    } else {
        LogPrintf("vaporum_addpair.maxpairs %d must be >= 0\n",(int32_t)maxpairs);
        return(-1);
    }
...
int32_t vaporum_MoMoMdata(char *hexstr,int32_t hexsize,struct vaporum_ccdataMoMoM *mdata,char *symbol,int32_t kmdheight,int32_t notarized_height)
{
    uint8_t hexdata[8192]; struct vaporum_ccdata *ccdata,*tmpptr; int32_t len,maxpairs,i,retval=-1,depth,starti,endi,CCid=0; struct vaporum_ccdata_entry *allMoMs;
    ...
    {
        if ( (allMoMs= vaporum_allMoMs(&depth,&mdata->MoMoM,starti,endi)) != 0 )
        {
            mdata->MoMoMdepth = depth;
            for (i=0; i<depth; i++)
            {
                if ( strcmp(symbol,allMoMs[i].symbol) == 0 )
                    maxpairs = vaporum_addpair(mdata,allMoMs[i].notarized_height,i,maxpairs);
            }
            if ( mdata->numpairs > 0 )
            {
...
void vaporum_purge_ccdata(int32_t height)
{
