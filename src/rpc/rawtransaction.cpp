...
void TxToJSONExpanded(const CTransaction& tx, const uint256 hashBlock, UniValue& entry, int nHeight = 0, int nConfirmations = 0, int nBlockTime = 0)
{
    ...
    if (!hashBlock.IsNull()) {
        ...
        if (nConfirmations > 0) {
            ...
            entry.push_back(Pair("confirmations", vaporum_dpowconfs(nHeight,nConfirmations)));
    ...
    if (!hashBlock.IsNull()) {
        ...
        if (mi != mapBlockIndex.end() && (*mi).second) {
            ...
            if (chainActive.Contains(pindex)) {
                ...
                entry.push_back(Pair("confirmations", vaporum_dpowconfs(pindex->nHeight,1 + chainActive.Height() - pindex->nHeight)));
...
UniValue createrawtransaction(const UniValue& params, bool fHelp, const CPubKey& mypk)
{
    ...
    if (fHelp || params.size() < 2 || params.size() > 4)
        throw runtime_error(
            ...
            "    {\n"
            "      \"address\": x.xxx,    (numeric or string, required) The key is the vaporum address or script (in hex), the numeric value (can be string) is the " + CURRENCY_UNIT + " amount\n"
...
UniValue decoderawtransaction(const UniValue& params, bool fHelp, const CPubKey& mypk)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            ...
            "         \"type\" : \"pubkeyhash\",  (string) The type, eg 'pubkeyhash'\n"
            "         \"addresses\" : [           (json array of string)\n"
            "           \"RTZMZHDFSTFQst8XmX2dR4DaH87cEUs3gC\"   (string) vaporum address\n"
            "           ,...\n"
            "         ]\n"
            "       }\n"
            "     }\n"
...
UniValue decodescript(const UniValue& params, bool fHelp, const CPubKey& mypk)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            ...
            "  \"addresses\": [   (json array of string)\n"
            "     \"address\"     (string) Vaporum address\n"
            "     ,...\n"
            "  ],\n"
