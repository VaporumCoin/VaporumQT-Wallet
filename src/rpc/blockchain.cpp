...
UniValue blockheaderToJSON(const CBlockIndex* blockindex)
{
    ...
    result.push_back(Pair("confirmations", vaporum_dpowconfs(blockindex->nHeight,confirmations)));
    ...
    result.push_back(Pair("segid", (int)vaporum_segid(0,blockindex->nHeight)));
...
UniValue getlastsegidstakes(const UniValue& params, bool fHelp, const CPubKey& mypk)
{
    ...
    {
        int8_t segid = vaporum_segid(0,i);
...
/*uint256 _vaporum_getblockhash(int32_t nHeight)
{
    uint256 hash;
    LOCK(cs_main);
    if ( nHeight >= 0 && nHeight <= chainActive.Height() )
...
UniValue kvsearch(const UniValue& params, bool fHelp, const CPubKey& mypk)
{
    ...
    if ( (keylen= (int32_t)strlen(params[0].get_str().c_str())) > 0 )
    {
        ...
        if ( keylen < sizeof(key) )
        {
            ...
            if ( (valuesize= vaporum_kvsearch(&refpubkey,chainActive.Tip()->nHeight,&flags,&height,value,key,keylen)) >= 0 )
            {
...
UniValue minerids(const UniValue& params, bool fHelp, const CPubKey& mypk)
{
    ...
    if ( (n= vaporum_minerids(minerids,height,(int32_t)(sizeof(minerids)/sizeof(*minerids)))) > 0 )
    {
        memset(tally,0,sizeof(tally));
        numnotaries = vaporum_notaries(pubkeys,height,timestamp);
        if ( numnotaries > 0 )
        {
            for (i=0; i<n; i++)
            {
...
UniValue notaries(const UniValue& params, bool fHelp, const CPubKey& mypk)
{
    ...
    if ( (n= vaporum_notaries(pubkeys,height,timestamp)) > 0 )
    {
        for (i=0; i<n; i++)
        {
            UniValue item(UniValue::VOBJ);
...
UniValue gettxout(const UniValue& params, bool fHelp, const CPubKey& mypk)
{
    if (fHelp || params.size() < 2 || params.size() > 3)
        throw runtime_error(
            ...
            "     \"addresses\" : [          (array of string) array of Vaporum addresses\n"
            "        \"vaporumaddress\"        (string) Vaporum address\n"
            "        ,...\n"
            "     ]\n"
            "  },\n"
    ...
    if ((unsigned int)coins.nHeight == MEMPOOL_HEIGHT) {
        ...
        ret.push_back(Pair("confirmations", vaporum_dpowconfs(coins.nHeight,pindex->nHeight - coins.nHeight + 1)));
    ...
    if ( (interest= vaporum_accrued_interest(&txheight,&locktime,hash,n,coins.nHeight,coins.vout[n].nValue,(int32_t)pindex->nHeight)) != 0 )
