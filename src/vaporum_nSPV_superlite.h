...
#ifndef VAPORUM_NSPVSUPERLITE_H
#define VAPORUM_NSPVSUPERLITE_H
...
// vaporum_nSPVresp is called from async message processing

void vaporum_nSPVresp(CNode *pfrom,std::vector<uint8_t> response) // received a response
{
...
// vaporum_nSPV from main polling loop (really this belongs in its own file, but it is so small, it ended up here)

void vaporum_nSPV(CNode *pto) // polling loop from SendMessages
{
...
UniValue NSPV_getinfo_json(struct NSPV_inforesp *ptr)
{
    ...
    result.push_back(Pair("result","success"));
    result.push_back(Pair("nSPV",VAPORUM_NSPV==-1?"disabled":(VAPORUM_NSPV_SUPERLITE?"superlite":"fullnode")));
