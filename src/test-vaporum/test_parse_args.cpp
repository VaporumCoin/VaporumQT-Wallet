...
namespace ParseArgumentsTests {
    ...
    class ParseArgumentsTests: public ::testing::Test {
        ...
        public:
            ...
            void SetUp( ) {
                ...
                STAKED_NOTARY_ID = -1; // should be set via vaporum_args call in real world
                SelectParams(CBaseChainParams::MAIN); // by default it's a CBaseChainParams::REGTEST, see ./src/test-vaporum/main.cpp
                chainName = assetchain();

                vaporum_setactivation(Consensus::NetworkUpgrade::ALWAYS_ACTIVE); // act as UpdateNetworkUpgradeParameters for regtest, to set sapling & overwinter activation height, but for mainnet
            ...
            void TearDown( ) {
                ...
                chainActive.SetTip(nullptr);
                vaporum_setactivation(Consensus::NetworkUpgrade::NO_ACTIVATION_HEIGHT);
    ...
    TEST_F(ParseArgumentsTests, ParseCommandLineArgs) {
        ...
        const char program_name[] = "vaporum-test";
        ...
        for(const auto& pair : mapKnownAssetchains) {
            ...
            ParseParameters(argc, argv.get()); // before calling vaporum_args -ac_name param should be set in mapArgs
            vaporum_args(argv0Data.get());      // argv0 is passed in try to get ac_name from program suffixes (works for MNZ and BTCH only)
