...
const int vaporum_interest_height = 247205+1;
...
public:
    ...
    bool GetCoins(const uint256 &txid, CCoins &coins) const override {
        if (sAllowedTxIn.count(txid)) {
            ...
            newCoins.nHeight = vaporum_interest_height-1; /* TODO: return correct nHeight depends on txid */
            coins.swap(newCoins);
            return true;
        }
        return false;
...
class VaporumFeatures : public ::testing::Test {
protected:
    virtual void SetUp() {
...
// some vaporum consensus extensions
TEST_F(VaporumFeatures, vaporum_interest_validate) {

    // Add a fake transaction to the wallet
    CMutableTransaction mtx0 = CreateNewContextualCMutableTransaction(Params().GetConsensus(), vaporum_interest_height-1);
    ...
    pfakeIndex->nHeight = vaporum_interest_height-1;
    ...
    EXPECT_EQ(vaporum_interest_height-1, chainActive.Height());
    ...
    bool missingInputs;
    CMutableTransaction mtxSpend = CreateNewContextualCMutableTransaction(Params().GetConsensus(), vaporum_interest_height);
    ...
    // create coinbase
    CMutableTransaction txcb = CreateNewContextualCMutableTransaction(Params().GetConsensus(), vaporum_interest_height);
    txcb.vin.resize(1);
    txcb.vin[0].prevout.SetNull();
    txcb.vin[0].scriptSig = (CScript() << vaporum_interest_height << CScriptNum(1)) + COINBASE_FLAGS;
    txcb.vout.resize(1);
    txcb.vout[0].scriptPubKey = GetScriptForDestination(DecodeDestination(testaddr));;
    txcb.vout[0].nValue = GetBlockSubsidy(vaporum_interest_height, Params().GetConsensus()) + 0;
...

    {
        ...
        EXPECT_EQ(state1.GetRejectReason(), "vaporum-interest-invalid");
    }
    {
...

    {
        ...
        for (size_t idx_ht = 0; idx_ht < sizeof(testHeights) / sizeof(testHeights[0]); ++idx_ht)
        {
            ...
            for (size_t idx = 0; idx < nMaxTipTimes; ++idx)
            {
                ...
                // bcz CCoinsViewCache::GetValueIn will call vaporum_accrued_interest
                // -> vaporum_interest_args -> GetTransaction
                ...
                // force vaporum_getblockindex in vaporum_interest_args return a value
                uint256 zero;
                zero.SetNull();
...
// check vaporum_interestnew calculations
TEST_F(VaporumFeatures, vaporum_interestnew) {
    ...
    EXPECT_EQ(vaporum_interestnew(7777777-1, 10LL*COIN, 1663839248, 1663839248 + (31 * 24 * 60 - 1) * 60 + 3600), 10LL*COIN/10512000 * (31*24*60 - 59) / 500);
    // end of interest era
    EXPECT_EQ(vaporum_interestnew(7777777, 10LL*COIN, 1663839248, 1663839248 + (31 * 24 * 60 - 1) * 60 + 3600), 0LL); 

    // value less than limit
    EXPECT_EQ(vaporum_interestnew(1000000, 10LL*COIN-1, 1663839248, 1663839248 + (31 * 24 * 60 - 1) * 60 + 3600), 0); 
    // tip less than nLockTime
    EXPECT_EQ(vaporum_interestnew(1000000, 10LL*COIN-1, 1663839248, 1663839248 - 1), 0); 
    // not timestamp value
    EXPECT_EQ(vaporum_interestnew(1000000, 10LL*COIN-1, 400000000U, 400000000U + 30 * 24 * 60 * 60 + 3600), 0); 

    // too small period
    EXPECT_EQ(vaporum_interestnew(1000000, 10LL*COIN, 1663839248, 1663839248 + 3600 - 1), 0); 
    // time over cut off month time limit
    EXPECT_EQ(vaporum_interestnew(1000000, 10LL*COIN, 1663839248, 1663839248 + 31 * 24 * 60 * 60 + 3600+1), 10LL*COIN/10512000 * (31*24*60 - 59)); 
    EXPECT_EQ(vaporum_interestnew(1000000, 10LL*COIN, 1663839248, 1663839248 + 32 * 24 * 60 * 60 + 3600), 10LL*COIN/10512000 * (31*24*60 - 59)); 

    // time close to cut off year time limit 
    EXPECT_EQ(vaporum_interestnew(1000000-1, 10LL*COIN, 1663839248, 1663839248 + (365 * 24 * 60 - 1) * 60 + 3600), 10LL*COIN/10512000 * (365*24*60 - 59)); 
    // time over cut off year time limit 
    EXPECT_EQ(vaporum_interestnew(1000000-1, 10LL*COIN, 1663839248, 1663839248 + (365 * 24 * 60 - 1) * 60 + 3600 + 60), 10LL*COIN/10512000 * (365*24*60 - 59)); 
    EXPECT_EQ(vaporum_interestnew(1000000-1, 10LL*COIN, 1663839248, 1663839248 + (365 * 24 * 60 - 1) * 60 + 3600 + 30 * 24 * 60), 10LL*COIN/10512000 * (365*24*60 - 59)); 
}

// check vaporum_interest calculations
TEST_F(VaporumFeatures, vaporum_interest) {

    const uint32_t activation = 1491350400;  // 1491350400 5th April

    {
        // some not working values should produce 0LL
        EXPECT_EQ(vaporum_interest(1, 1000LL, 1, 1), 0LL); 
    }
    {
        // nValue <= 25000LL*COIN and nValue >= 25000LL*COIN
        // txheight >= 1000000 
        // should be routed to vaporum_interestnew

        for (CAmount nValue : { 10LL*COIN, 25001LL*COIN })
        {
            // time lower than cut off month time limit
            EXPECT_EQ(vaporum_interest(1000000, nValue, 1663839248, 1663839248 + (31 * 24 * 60 - 1) * 60 + 3600), nValue/10512000 * (31*24*60 - 59)); 

            // since 7th season, according to KIP0001 AUR should be reduced from 5% to 0.01%, i.e. div by 500
            EXPECT_EQ(vaporum_interest(7777777-1, nValue, 1663839248, 1663839248 + (31 * 24 * 60 - 1) * 60 + 3600), nValue/10512000 * (31*24*60 - 59) / 500);
            // end of interest era
            ...
            // not timestamp value
            EXPECT_EQ(vaporum_interest(1000000, nValue-1, 400000000U, 400000000U + 30 * 24 * 60 * 60 + 3600), 0); 

            // too small period
            EXPECT_EQ(vaporum_interest(1000000, nValue, 1663839248, 1663839248 + 3600 - 1), 0); 
            // time over cut off month time limit
            EXPECT_EQ(vaporum_interest(1000000, nValue, 1663839248, 1663839248 + 31 * 24 * 60 * 60 + 3600+1), nValue/10512000 * (31*24*60 - 59)); 
            EXPECT_EQ(vaporum_interest(1000000, nValue, 1663839248, 1663839248 + 32 * 24 * 60 * 60 + 3600), nValue/10512000 * (31*24*60 - 59)); 
        }
        // value less than limit
        EXPECT_EQ(vaporum_interest(1000000, 10LL*COIN-1, 1663839248, 1663839248 + (31 * 24 * 60 - 1) * 60 + 3600), 0); 
    }

    for (auto days : { 1, 10, 365, 365*2, 365*3 })
    {
        std::cerr << "running vaporum_interest test for days=" << days << "..." << std::endl;
            ...
            minutes = 365 * 24 * 60;
        {
            ...
            EXPECT_EQ(vaporum_interest(1000000-1, 10LL*COIN, 1663839248, 1663839248 + minutes * 60), numerator * (minutes - 59) / (365ULL * 24 * 60)); 
        }
        {
            // nValue <= 25000LL*COIN
            // txheight < 250000 
            ...
            EXPECT_EQ(vaporum_interest(250000-1, 10LL*COIN, locktime, tiptime), numerator / denominator / COIN); 
        }
        {
            // !exception
            // nValue > 25000LL*COIN
            ...
            denominator = (denominator == 0LL) ? 1LL : denominator;
            EXPECT_EQ(vaporum_interest(250000-1, 25000LL*COIN+1, 1663839248, 1663839248 + minutes * 60), numerator / denominator); 
        }
        {
            // !exception
            // nValue > 25000LL*COIN
            ...
            EXPECT_EQ(vaporum_interest(1000000-1, 25000LL*COIN+1, 1663839248, 1663839248 + minutes * 60), numerator * minutes_adj / (365LL * 24 * 60)); 
        }
        {
            // exception
            // nValue > 25000LL*COIN
            ...
            for (const auto htval : std::vector<std::pair<int32_t, CAmount>>{ {116607, 2502721100000LL}, {126891, 2879650000000LL}, {129510, 3000000000000LL}, {141549, 3500000000000LL}, {154473, 3983399350000LL}, {154736, 3983406748175LL}, {155013, 3983414006565LL}, {155492, 3983427592291LL}, {155613, 9997409999999797LL}, {157927, 9997410667451072LL}, {155613, 2590000000000LL}, {155949, 4000000000000LL} })
            {
                ...
                denominator = (denominator == 0LL) ? 1LL : denominator;
                if (txheight < 155949)
                    EXPECT_EQ(vaporum_interest(txheight, nValue, locktime, tiptime), numerator / denominator / COIN); 
            }
        }
    }
}
