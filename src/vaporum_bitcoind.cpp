...
#include "vaporum.h" // vaporum_voutupdate()
#include "vaporum_utils.h" // OS_milliseconds
...
char *vaporum_issuemethod(char *userpass,char *method,char *params,uint16_t port)
{
    //static void *cHandle;
...
int32_t notarizedtxid_height(char *dest,char *txidstr,int32_t *kmdnotarized_heightp)
{
    ...
    *kmdnotarized_heightp = 0;
    ...
    if ( userpass[0] != 0 )
    {
        ...
        {
            if ( (jsonstr= vaporum_issuemethod(userpass,(char *)"getinfo",params,port)) != 0 )
            {
                //LogPrintf("(%s)\n",jsonstr);
            ...
            if ( (jsonstr= vaporum_issuemethod(userpass,(char *)"getblockchaininfo",params,port)) != 0 )
            {
                //LogPrintf("(%s)\n",jsonstr);
                if ( (json= cJSON_Parse(jsonstr)) != 0 )
                {
        ...
        if ( (jsonstr= vaporum_issuemethod(userpass,(char *)"getrawtransaction",params,port)) != 0 )
        {
            //LogPrintf("(%s)\n",jsonstr);
            if ( (json= cJSON_Parse(jsonstr)) != 0 )
            {
...
int32_t vaporum_verifynotarizedscript(int32_t height,uint8_t *script,int32_t len,uint256 NOTARIZED_HASH)
{
...
void vaporum_reconsiderblock(uint256 blockhash)
{
    char params[256],*jsonstr,*hexstr;
    sprintf(params,"[\"%s\"]",blockhash.ToString().c_str());
    if ( (jsonstr= vaporum_issuemethod(ASSETCHAINS_USERPASS,(char *)"reconsiderblock",params,ASSETCHAINS_RPCPORT)) != 0 )
    {
        //LogPrintf("vaporum_reconsiderblock.(%s) (%s %u) -> (%s)\n",params,ASSETCHAINS_USERPASS,ASSETCHAINS_RPCPORT,jsonstr);
        free(jsonstr);
    }
    //LogPrintf("vaporum_reconsiderblock.(%s) (%s %u) -> NULL\n",params,ASSETCHAINS_USERPASS,ASSETCHAINS_RPCPORT);
}

int32_t vaporum_verifynotarization(const char *symbol,const char *dest,int32_t height,int32_t NOTARIZED_HEIGHT,uint256 NOTARIZED_HASH,uint256 NOTARIZED_DESTTXID)
{
    char params[256];
    ...
    else if ( strcmp(dest,"BTC") == 0 )     // Note: this should work for LTC too (BTC is used as an alias for LTC)
    {
        if ( BTCUSERPASS[0] != 0 )
        {
            jsonstr = vaporum_issuemethod(BTCUSERPASS,(char *)"getrawtransaction",params,DEST_PORT);
        }
        //else jsonstr = _dex_getrawtransaction();
        else return(0);
    }
            ...
            if ( (txjson= jobj(json,(char *)"result")) != 0 && (vouts= jarray(&n,txjson,(char *)"vout")) != nullptr )
            {
                ...
                if ( (skey= jobj(vout,(char *)"scriptPubKey")) != 0 )
                {
                    if ( (hexstr= jstr(skey,(char *)"hex")) != 0 )
                    {
                        ...
                        retval = vaporum_verifynotarizedscript(height,script,len,NOTARIZED_HASH);
                    }
                }
            }
            free_json(txjson);
...
uint64_t vaporum_seed(int32_t height)
{
    uint64_t seed = 0;
    /*if ( 0 ) // problem during init time, seeds are needed for loading blockindex, so null seeds...
     ...
     hash = _vaporum_getblockhash(height);
     if ( memcmp(&hash,&zero,sizeof(hash)) == 0 )
     hash = vaporum_getblockhash(height);
     int32_t i;
     for (i=0; i<32; i++)
     LogPrintf("%02x",((uint8_t *)&hash)[i]);
     LogPrintf(" seed.%d\n",height);
...
uint32_t vaporum_txtime(CScript &opret,uint64_t *valuep,uint256 hash, int32_t n, char *destaddr)
{
...
CBlockIndex *vaporum_getblockindex(uint256 hash)
{
    BlockMap::const_iterator it = mapBlockIndex.find(hash);
    return((it != mapBlockIndex.end()) ? it->second : NULL);
}

uint32_t vaporum_txtime2(uint64_t *valuep,uint256 hash,int32_t n,char *destaddr)
{
    ...
    *valuep = 0;
    ...
    if ( (pindex= vaporum_getblockindex(hashBlock)) != 0 )
        txtime = pindex->nTime;
...
int32_t vaporum_newStakerActive(int32_t height, uint32_t timestamp)
{
    if ( timestamp > nStakedDecemberHardforkTimestamp || vaporum_heightstamp(height) > nStakedDecemberHardforkTimestamp ) //December 2019 hardfork
        return(1);
    else return(0);
}

int32_t vaporum_hasOpRet(int32_t height, uint32_t timestamp)
{    
    return vaporum_newStakerActive(height, timestamp) == 1;
}

bool vaporum_checkopret(CBlock *pblock, CScript &merkleroot)
{
    merkleroot = pblock->vtx.back().vout.back().scriptPubKey;
    return(merkleroot.IsOpReturn() && merkleroot == vaporum_makeopret(pblock, false));
}

bool vaporum_hardfork_active(uint32_t time)
{
...
uint256 vaporum_calcmerkleroot(CBlock *pblock, uint256 prevBlockHash, int32_t nHeight, bool fNew, CScript scriptPubKey)
{
    std::vector<uint256> vLeaves;
    // rereate coinbase tx
...
int32_t vaporum_isPoS(CBlock *pblock, int32_t height,CTxDestination *addressout)
{
    ...
    if ( ASSETCHAINS_STAKED != 0 )
    {
        ...
        if ( n > 1 && pblock->vtx[n-1].vin.size() == 1 && pblock->vtx[n-1].vout.size() == 1+vaporum_hasOpRet(height,pblock->nTime) )
        {
            txid = pblock->vtx[n-1].vin[0].prevout.hash;
            vout = pblock->vtx[n-1].vin[0].prevout.n;
            txtime = vaporum_txtime(opret,&value,txid,vout,destaddr);
            if ( ExtractDestination(pblock->vtx[n-1].vout[0].scriptPubKey,voutaddress) )
            {
                ...
                if ( vaporum_newStakerActive(height, pblock->nTime) != 0 ) 
                {
                    if ( DecodeStakingOpRet(pblock->vtx[n-1].vout[1].scriptPubKey, merkleroot) != 0 && vaporum_calcmerkleroot(pblock, pblock->hashPrevBlock, height, false, pblock->vtx[0].vout[0].scriptPubKey) == merkleroot )
                    {
                        return(1);
                    }
                }
...
int32_t vaporum_is_notarytx(const CTransaction& tx)
{
...
int32_t vaporum_block2height(CBlock *block)
{
...
int32_t vaporum_blockload(CBlock& block,CBlockIndex *pindex)
{
    block.SetNull();
...
CBlockIndex *vaporum_chainactive(int32_t height)
{
    AssertLockHeld(cs_main);
    CBlockIndex *index = chainActive.Tip();
    if ( index != nullptr )
...
uint32_t vaporum_heightstamp(int32_t height)
{
    CBlockIndex *ptr;
    if ( height > 0 && (ptr= vaporum_chainactive(height)) != nullptr )
        return(ptr->nTime);
    return 0;
}
...
void vaporum_index2pubkey33(uint8_t *pubkey33,CBlockIndex *pindex,int32_t height)
{
    ...
    if ( pindex != 0 )
    {
        if ( vaporum_blockload(block,pindex) == 0 )
            vaporum_block2pubkey33(pubkey33,&block);
    }
}
...
int32_t vaporum_eligiblenotary(uint8_t pubkeys[66][33],int32_t *mids,uint32_t blocktimes[66],int32_t *nonzpkeysp,int32_t height)
{
    ...
    n = vaporum_notaries(notarypubs33,height,0);
    for (i=duplicate=0; i<66; i++)
    {
        if ( (pindex= vaporum_chainactive(height-i)) != 0 )
        {
            blocktimes[i] = pindex->nTime;
            if ( vaporum_blockload(block,pindex) == 0 )
            {
                vaporum_block2pubkey33(pubkeys[i],&block);
                for (j=0; j<n; j++)
                {
...
int32_t vaporum_minerids(uint8_t *minerids,int32_t height,int32_t width)
{
    int32_t i,j,nonz,numnotaries; CBlock block; CBlockIndex *pindex; uint8_t notarypubs33[64][33],pubkey33[33];
    numnotaries = vaporum_notaries(notarypubs33,height,0);
    for (i=nonz=0; i<width; i++)
    {
        if ( height-i <= 0 )
            continue;
        if ( (pindex= vaporum_chainactive(height-width+i+1)) != 0 )
        {
            if ( vaporum_blockload(block,pindex) == 0 )
            {
                vaporum_block2pubkey33(pubkey33,&block);
...
int32_t vaporum_is_special(uint8_t pubkeys[66][33],int32_t mids[66],uint32_t blocktimes[66],int32_t height,uint8_t pubkey33[33],uint32_t blocktime)
{
    int32_t i,j,notaryid=0,minerid,limit,nid; uint8_t destpubkey33[33];
    vaporum_chosennotary(&notaryid,height,pubkey33,blocktimes[0]);
    if ( height >= 82000 )
    {
        if ( notaryid >= 0 )
        {
    ...
    else
    {
        if ( height >= 34000 && notaryid >= 0 )
        {
            ...
            for (i=1; i<limit; i++)
            {
                vaporum_chosennotary(&nid,height-i,pubkey33,blocktimes[i]);
                if ( nid == notaryid )
                {
                    //for (j=0; j<66; j++)
                    //    LogPrintf("%d ",mids[j]);
...
int32_t vaporum_MoM(int32_t *notarized_heightp,uint256 *MoMp,uint256 *kmdtxidp,int32_t nHeight,uint256 *MoMoMp,int32_t *MoMoMoffsetp,int32_t *MoMoMdepthp,int32_t *kmdstartip,int32_t *kmdendip)
{
    int32_t depth,notarized_ht; uint256 MoM,kmdtxid;
    depth = vaporum_MoMdata(&notarized_ht,&MoM,&kmdtxid,nHeight,MoMoMp,MoMoMoffsetp,MoMoMdepthp,kmdstartip,kmdendip);
...
CBlockIndex *vaporum_blockindex(uint256 hash)
{
...
int32_t vaporum_blockheight(uint256 hash)
{
    BlockMap::const_iterator it; CBlockIndex *pindex = 0;
    if ( (it = mapBlockIndex.find(hash)) != mapBlockIndex.end() )
    {
...
uint32_t vaporum_blocktime(uint256 hash)
{
    BlockMap::const_iterator it; CBlockIndex *pindex = 0;
    if ( (it = mapBlockIndex.find(hash)) != mapBlockIndex.end() )
    {
 ...
 */
bool vaporum_checkpoint(int32_t *notarized_heightp, int32_t nHeight, uint256 hash)
{
    CBlockIndex *pindex;
    ...
    int32_t notarized_height = vaporum_notarizeddata(pindex->nHeight,&notarized_hash,&notarized_desttxid);
    *notarized_heightp = notarized_height;
    ...
    {
        //verify that the block info returned from vaporum_notarizeddata matches the actual block
...
int32_t vaporum_nextheight()
{
    ...
    if ( (pindex= chainActive.Tip()) != 0 && (ht= pindex->nHeight) > 0 )
        return(ht+1);
    else return(vaporum_longestchain() + 1);
}
 ...
 * @return 1 if this chain's height >= vaporum_longestchain(), otherwise 0
 */
int32_t vaporum_isrealtime(int32_t *kmdheightp)
{
    AssertLockHeld(cs_main);

    vaporum_state *sp = vaporum_stateptrget( (char*)"KMD");
    if ( sp != nullptr )
        *kmdheightp = sp->CURRENT_HEIGHT;
    else 
        *kmdheightp = 0;
    CBlockIndex *pindex = chainActive.Tip();
    if ( pindex != nullptr && pindex->nHeight >= (int32_t)vaporum_longestchain() )
        return 1;
    return 0;
}
...
/*
 vaporum_checkPOW (fast) is called early in the process and should only refer to data immediately available. it is a filter to prevent bad blocks from going into the local DB. The more blocks we can filter out at this stage, the less junk in the local DB that will just get purged later on.

 vaporum_checkPOW (slow) is called right before connecting blocks so all prior blocks can be assumed to be there and all checks must pass
...
uint64_t vaporum_commission(const CBlock *pblock,int32_t height)
{
...
uint32_t vaporum_segid32(char *coinaddr)
{
    bits256 addrhash;
...
int8_t vaporum_segid(int32_t nocache,int32_t height)
{
    ...
    if ( height > 0 && (pindex= vaporum_chainactive(height)) != 0 )
    {
        if ( nocache == 0 && pindex->segid >= -1 )
            return(pindex->segid);
        if ( vaporum_blockload(block,pindex) == 0 )
        {
            newStakerActive = vaporum_newStakerActive(height, block.nTime);
            txn_count = block.vtx.size();
            if ( txn_count > 1 && block.vtx[txn_count-1].vin.size() == 1 && block.vtx[txn_count-1].vout.size() == 1+vaporum_hasOpRet(height,pindex->nTime) )
            {
                ...
                if ( ExtractDestination(block.vtx[txn_count-1].vout[0].scriptPubKey,voutaddress) )
                {
                    ...
                    {
                        segid = vaporum_segid32(voutaddr) & 0x3f;
                        //LogPrintf( "vaporum_segid: ht.%i --> %i\n",height,pindex->segid);
                    }
                } //else LogPrintf("vaporum_segid ht.%d couldnt extract voutaddress\n",height);
            }
        }
        // The new staker sets segid in vaporum_checkPOW, this persists after restart by being saved in the blockindex for blocks past the HF timestamp, to keep backwards compatibility.
...
void vaporum_segids(uint8_t *hashbuf,int32_t height,int32_t n)
{
    ...
    else
    {
        ...
        for (i=0; i<n; i++)
        {
            hashbuf[i] = (uint8_t)vaporum_segid(0,height+i);
...
uint32_t vaporum_stakehash(uint256 *hashp,char *address,uint8_t *hashbuf,uint256 txid,int32_t vout)
{
    bits256 addrhash;
...
arith_uint256 vaporum_PoWtarget(int32_t *percPoSp,arith_uint256 target,int32_t height,int32_t goalperc,int32_t newStakerActive)
{
    ...
    *percPoSp = percPoS = 0;
    ...
    for (i=n=m=0; i<100; i++)
    {
        ...
        if ( ht <= 1 )
            continue;
        if ( (pindex= vaporum_chainactive(ht)) != 0 )
        {
            if ( vaporum_segid(0,ht) >= 0 )
            {
                n++;
                percPoS++;
...
uint32_t vaporum_stake(int32_t validateflag,arith_uint256 bnTarget,int32_t nHeight,uint256 txid,int32_t vout,uint32_t blocktime,uint32_t prevtime,char *destaddr,int32_t PoSperc)
{
    ...
    txtime = vaporum_txtime2(&value,txid,vout,address);
    if ( validateflag == 0 )
    {
    ...
    {
        //LogPrintf("vaporum_stake null %.8f %u %u %u\n",dstr(value),txtime,blocktime,prevtime);
        return(0);
    }
    if ( value < SATOSHIDEN )
        return(0);
    ...
    if ( (minage= nHeight*3) > 6000 ) // about 100 blocks
        minage = 6000;
    vaporum_segids(hashbuf,nHeight-101,100);
    segid32 = vaporum_stakehash(&hash,address,hashbuf,txid,vout);
...
int32_t vaporum_is_PoSblock(int32_t slowflag,int32_t height,CBlock *pblock,arith_uint256 bnTarget,arith_uint256 bhash)
{
    ...
    int32_t newStakerActive = vaporum_newStakerActive(height, pblock->nTime);
    // Get PoSperc and POW Target. slowflag only here, calling it when blocks out of order causes problems.
    if ( slowflag != 0 )
    {
        POWTarget = vaporum_PoWtarget(&PoSperc,bnTarget,height,ASSETCHAINS_STAKED,newStakerActive);
    }
    else 
    {
        // checks opret merkle root and existence of staking tx.
        return(vaporum_isPoS(pblock,height,0));
    }
    txn_count = pblock->vtx.size();
    ...
    if ( txn_count > 1 && pblock->vtx[txn_count-1].vin.size() == 1 && pblock->vtx[txn_count-1].vout.size() == 1+vaporum_hasOpRet(height,pblock->nTime) )
    {
        ...
        {
            if ( vaporum_isPoS(pblock,height,0) != 0 ) 
            {
                // checks utxo is eligible to stake this block
                eligible = vaporum_stake(1,bnTarget,height,txid,vout,pblock->nTime,prevtime+ASSETCHAINS_STAKED_BLOCK_FUTURE_HALF,(char *)"",PoSperc); 
            }
            if ( eligible == 0 || eligible > pblock->nTime )
            {
                if ( 0 && ASSETCHAINS_STAKED < 100 ) 
                    LogPrintf("vaporum_is_PoSblock PoS failure ht.%d eligible.%u vs blocktime.%u, lag.%d -> check to see if it is PoW block\n",height,eligible,(uint32_t)pblock->nTime,(int32_t)(eligible - pblock->nTime));
            }
            else 
            {
                isPoS = 1;
...
uint64_t vaporum_notarypayamount(int32_t nHeight, int64_t notarycount)
{
    ...
    if ( notarycount == 0 )
    {
        LogPrintf( "vaporum_notarypayamount failed num notaries is 0!\n");
        return(0);
    }
...
int32_t vaporum_getnotarizedheight(uint32_t timestamp,int32_t height, uint8_t *script, int32_t len)
{
    ...
    if ( len >= sizeof(uint32_t) && len <= sizeof(scriptbuf) )
    {
        ...
        if ( vaporum_voutupdate(true,&isratification,0,scriptbuf,len,height,uint256(),1,1,&voutmask,&specialtx,&notarizedheight,0,1,0,timestamp) != -2 )
        {
...
uint64_t vaporum_notarypay(CMutableTransaction &txNew, std::vector<int8_t> &NotarisationNotaries, uint32_t timestamp, int32_t height, uint8_t *script, int32_t len)
{
    // fetch notary pubkey array.
    uint64_t total = 0, AmountToPay = 0;
    int8_t numSN = 0; uint8_t notarypubkeys[64][33] = {0};
    numSN = vaporum_notaries(notarypubkeys, height, timestamp);
    ...
    int32_t notarizedheight = vaporum_getnotarizedheight(timestamp, height, script, len);
    ...
    AmountToPay = vaporum_notarypayamount(height,NotarisationNotaries.size());
    if ( AmountToPay == 0 )
        return(0);
...
uint64_t vaporum_checknotarypay(CBlock *pblock,int32_t height)
{
    ...
    if ( pblock->vtx[1].vout.size() == 2 && pblock->vtx[1].vout[1].nValue == 0 )
    {
        ...
        if ( script[0] == OP_RETURN )
        {
            ...
            totalsats = vaporum_notarypay(txNew, NotarisationNotaries, pblock->nTime, height, script, scriptlen);
        } 
        else 
        {
...
bool vaporum_appendACscriptpub()
{
...
int32_t vaporum_checkPOW(int64_t stakeTxValue, int32_t slowflag,CBlock *pblock,int32_t height)
{
    ...
    possible = vaporum_block2pubkey33(pubkey33,pblock);
    if ( height == 0 )
    {
        if ( slowflag != 0 )
        {
    ...
    if ( ( !chainName.isKMD() || height > 792000) && bhash > bnTarget )
    {
        ...
        {
            if ( (n= vaporum_notaries(pubkeys,height,pblock->nTime)) > 0 )
            {
                for (i=0; i<n; i++)
    ...
    if ( ASSETCHAINS_STAKED != 0 && height >= 2 ) // must PoS or have at least 16x better PoW
    {
        ...
        pindex = it != mapBlockIndex.end() ? it->second : NULL;
        int32_t newStakerActive = vaporum_newStakerActive(height, pblock->nTime);
        if ( (is_PoSblock= vaporum_is_PoSblock(slowflag,height,pblock,bnTarget,bhash)) == 0 )
        {
            ...
            {
                bnTarget = vaporum_PoWtarget(&PoSperc,bnTarget,height,ASSETCHAINS_STAKED,newStakerActive);
                if ( bhash > bnTarget ) 
                {
                    for (i=31; i>=16; i--)
    ...
    if ( failed == 0 && ASSETCHAINS_COMMISSION != 0 ) 
    {
        ...
        {
            if ( vaporum_checkcommission(pblock,height) < 0 )
                return(-1);
        }
    }
    ...
    if ( failed == 0 && ASSETCHAINS_NOTARY_PAY[0] != 0 && pblock->vtx.size() > 1 )
    {
        ...
        if ( slowflag == 0 && pblock->vtx[0].vout.size() > 1 )
        {
            ...
            if ( !vaporum_is_notarytx(pblock->vtx[1]) )
            {
            ...
            // Check min sigs.
            int8_t numSN = 0; uint8_t notarypubkeys[64][33] = {0};
            numSN = vaporum_notaries(notarypubkeys, height, pblock->nTime);
...
//LogPrintf("vaporum_checkPOW possible.%d slowflag.%d ht.%d notaryid.%d failed.%d\n",possible,slowflag,height,notaryid,failed);
    if ( failed != 0 && possible == 0 && notaryid < 0 )
        return(-1);
    else return(0);
}

int32_t vaporum_acpublic(uint32_t tiptime)
{
...
int64_t vaporum_newcoins(int64_t *zfundsp,int64_t *sproutfundsp,int32_t nHeight,CBlock *pblock)
{
...
int64_t vaporum_coinsupply(int64_t *zfundsp,int64_t *sproutfundsp,int32_t height)
{
    CBlockIndex *pindex; CBlock block; int64_t zfunds=0,sproutfunds=0,supply = 0;
    //LogPrintf("coinsupply %d\n",height);
    *zfundsp = *sproutfundsp = 0;
    if ( (pindex= vaporum_chainactive(height)) != 0 )
    {
        while ( pindex != 0 && pindex->nHeight > 0 )
        {
            ...
            {
                if ( vaporum_blockload(block,pindex) == 0 )
                    pindex->newcoins = vaporum_newcoins(&pindex->zfunds,&pindex->sproutfunds,pindex->nHeight,&block);
                else
                {
...
void vaporum_addutxo(std::vector<vaporum_staking> &array,uint32_t txtime,uint64_t nValue,uint256 txid,int32_t vout,char *address,uint8_t *hashbuf,CScript pk)
{
    uint256 hash; uint32_t segid32; vaporum_staking kp;
    segid32 = vaporum_stakehash(&hash,address,hashbuf,txid,vout);
...
int32_t vaporum_staked(CMutableTransaction &txNew,uint32_t nBits,uint32_t *blocktimep,uint32_t *txtimep,uint256 *utxotxidp,int32_t *utxovoutp,uint64_t *utxovaluep,uint8_t *utxosig, uint256 merkleroot)
{
    // use thread_local to prevent crash in case of accidental thread overlapping
    thread_local std::vector<vaporum_staking> array; 
    ...
    *utxovaluep = 0;
    ...
    //tmpTarget = vaporum_PoWtarget(&PoSperc,bnTarget,nHeight,ASSETCHAINS_STAKED);
    ...
    if ( array.size() != 0 )
    {
        ...
        if ( ReadBlockFromDisk(block, pblockindex, 1) && vaporum_isPoS(&block, nHeight, &addressout) != 0 && IsMine(*pwalletMain,addressout) != 0 )
        {
              resetstaker = true;
    ...
    if ( resetstaker || array.size() == 0 || time(NULL) > lasttime+600 )
    {
        ...
        BOOST_FOREACH(const COutput& out, vecOutputs)
        {
            ...
            {
                //LogPrintf("vaporum_staked invalid depth %d\n",(int32_t)out.nDepth);
                continue;
            }
            ...
            if ( ExtractDestination(pk,address) != 0 )
            {
                if ( IsMine(*pwalletMain,address) == 0 )
                    continue;
                if ( myGetTransaction(out.tx->GetHash(),tx,hashBlock) != 0 && (pindex= vaporum_getblockindex(hashBlock)) != 0 )
                {
                    vaporum_addutxo(array,(uint32_t)pindex->nTime,(uint64_t)nValue,out.tx->GetHash(),out.i,(char *)CBitcoinAddress(address).ToString().c_str(),hashbuf,(CScript)pk);
    ...
    for (i=winners=0; i<array.size(); i++)
    {
        ...
        vaporum_staking &kp = array[i];
        eligible = vaporum_stake(0,bnTarget,nHeight,kp.txid,kp.vout,0,(uint32_t)tipindex->nTime+ASSETCHAINS_STAKED_BLOCK_FUTURE_HALF,kp.address,PoSperc);
        if ( eligible > 0 )
        {
            besttime = 0;
            if ( eligible == vaporum_stake(1,bnTarget,nHeight,kp.txid,kp.vout,eligible,(uint32_t)tipindex->nTime+ASSETCHAINS_STAKED_BLOCK_FUTURE_HALF,kp.address,PoSperc) )
            {
    ...
    if ( earliest != 0 )
    {
        ...
        if ( (newStakerActive= vaporum_newStakerActive(nHeight,earliest)) != 0 )
        {
