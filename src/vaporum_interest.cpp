...
uint64_t vaporum_interestnew(int32_t txheight,uint64_t nValue,uint32_t nLockTime,uint32_t tiptime)
{
    if ( txheight < VAPORUM_ENDOFERA 
            ...
            && nLockTime < tiptime 
            && nValue >= 10*COIN )
        return _vaporum_interestnew(txheight,nValue,nLockTime,tiptime);
    return 0;
}
 ...
 */
uint64_t vaporum_interest(int32_t txheight,uint64_t nValue,uint32_t nLockTime,uint32_t tiptime)
{
            ...
            if ( nValue > 25000LL*COIN )
            {
                ...
                if ( !exception )
                {
                    ...
                    else if ( txheight < 1000000 )
                    {
                        ...
                        interest = _vaporum_interestnew(txheight,nValue,nLockTime,tiptime);
                }
                else if ( txheight < 1000000 )
                {
 ...
 * @returns locktime
 */
uint32_t vaporum_interest_args(uint32_t *txheighttimep,int32_t *txheightp,uint32_t *tiptimep,uint64_t *valuep,
        uint256 hash,int32_t n)
{
    ...
    *valuep = 0;
    ...
    if ( n < tx.vout.size() )
    {
        CBlockIndex *pindex = vaporum_getblockindex(hashBlock);
        if ( pindex != nullptr )
        {
 ...
 */
uint64_t vaporum_accrued_interest(int32_t *txheightp,uint32_t *locktimep,uint256 hash,int32_t n,
        int32_t checkheight,uint64_t checkvalue,int32_t tipheight)
{
    ...
    uint64_t value;
    *locktimep = vaporum_interest_args(&txheighttimep, txheightp, &tiptime, &value, hash, n);
    if ( *locktimep != 0 )
    {
        if ( (checkvalue == 0 || value == checkvalue) && (checkheight == 0 || *txheightp == checkheight) )
            return vaporum_interest(*txheightp,value,*locktimep,tiptime);
        else 
            LogPrintf("vaporum_accrued_interest value mismatch %llu vs %llu or height mismatch %d vs %d\n",(long long)value,(long long)checkvalue,*txheightp,checkheight);
    }
    return 0;
}
