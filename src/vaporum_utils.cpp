...
uint64_t vaporum_block_prg(uint32_t nHeight)
{
...
int64_t vaporum_block_unlocktime(uint32_t nHeight)
{
    ...
    {
        unlocktime = vaporum_block_prg(nHeight) % (ASSETCHAINS_TIMEUNLOCKTO - ASSETCHAINS_TIMEUNLOCKFROM);
 ...
 * @return the RPC port
 */
uint16_t _vaporum_userpass(char *username,char *password,FILE *fp)
{
    uint16_t port = 0;
...
void vaporum_statefname(char *fname, const char *symbol, const char *str)
{
    int32_t n,len;
    ...
    if ( (n= (int32_t)chainName.symbol().size()) != 0 )
    {
        ...
        if ( !mapArgs.count("-datadir") && strcmp(chainName.symbol().c_str(),&fname[len - n]) == 0 )
            fname[len - n] = 0;
        else if(mapArgs.count("-datadir")) LogPrintf("DEBUG - vaporum_utils:1363: custom datadir\n");
        else
        {
...
void vaporum_configfile(const char *symbol,uint16_t rpcport)
{
    ...
    if ( symbol != 0 && rpcport != 0 )
    {
        ...
        {
            _vaporum_userpass(myusername,mypassword,fp);
...
uint16_t vaporum_userpass(char *userpass,const char *symbol)
{
    ...
    if ( strcmp("KMD",symbol) == 0 )
    {
        ...
        vaporum_statefname(fname,symbol,confname);
    } else sprintf(fname,"%s",GetDataDir().string().c_str());
    
    if ( (fp= fopen(fname,"rb")) != 0 )
    {
        port = _vaporum_userpass(username,password,fp);
 ...
 */
uint32_t vaporum_assetmagic(const char *symbol,uint64_t supply,uint8_t *extraptr,int32_t extralen)
{
    uint8_t buf[512]; 
    uint32_t crc0=0; 
    int32_t len = 0; 
 ...
 */
uint16_t vaporum_assetport(uint32_t magic,int32_t extralen)
{
    if ( magic == 0x8de4eef9 )
        return 7770;
    else if ( extralen == 0 )
 ...
 */
uint16_t vaporum_port(const char *symbol,uint64_t supply,uint32_t *magicp,uint8_t *extraptr,int32_t extralen)
{
    if ( symbol == 0 || symbol[0] == 0 || strcmp("KMD",symbol) == 0 )
    {
        *magicp = 0x8de4eef9;
    ...
    *magicp = vaporum_assetmagic(symbol,supply,extraptr,extralen);
    return vaporum_assetport(*magicp,extralen);
}


int32_t vaporum_whoami(char *pubkeystr,int32_t height,uint32_t timestamp)
{
    int32_t i,notaryid;
    for (i=0; i<33; i++)
        sprintf(&pubkeystr[i<<1],"%02x",NOTARY_PUBKEY33[i]);
    pubkeystr[66] = 0;
    vaporum_chosennotary(&notaryid,height,NOTARY_PUBKEY33,timestamp);
    return(notaryid);
}
...
uint64_t vaporum_max_money()
{
    return vaporum_current_supply(10000000);
}

uint64_t vaporum_ac_block_subsidy(int nHeight)
{
...
int8_t equihash_params_possible(uint64_t n, uint64_t k)
{
    ...
    *  EhOptimisedSolve
    *  EhIsValidSolution
    * Alternatively change ASSETCHAINS_N and ASSETCHAINS_K in vaporum_nk.h for fast testing.
    */
    if ( k == 9 && (n == 200 || n == 210) )
        return(0);
...
#ifdef __APPLE__
    std::string filename = "Vaporum.conf";
#else
    std::string filename = "vaporum.conf";
#endif
...
void vaporum_args(char *argv0)
{
    ...
    SoftSetArg("-ac_name", std::string("VPRM"));
    SoftSetArg("-ac_supply", std::string("0"));
    SoftSetArg("-ac_eras", std::string("6"));
    SoftSetArg("-ac_blocktime", std::string("30"));
    SoftSetArg("-ac_reward", std::string("5000000000,2500000000,1250000000,625000000,312500000,156250000"));
    SoftSetArg("-ac_end", std::string("1000000,3500000,8500000,18500000,38500000,166500000"));
    ...
    SoftSetArg("-addnode", std::string("137.184.78.31"));
    ...
    if ( !chainName.isKMD() )
    {
        ...
        MAX_MONEY = vaporum_max_money();
        int32_t baseid;
        if ( (baseid = vaporum_baseid(chainName.symbol().c_str())) >= 0 && baseid < 32 )
        {
        ...
        if ( !chainName.isKMD() )
        {
            ...
            uint16_t port;
            if ( (port= vaporum_userpass(ASSETCHAINS_USERPASS,chainName.symbol().c_str())) != 0 )
                ASSETCHAINS_RPCPORT = port;
            else vaporum_configfile(chainName.symbol().c_str(),ASSETCHAINS_P2PPORT + 1);
...
/****
 * @brief get the right vaporum_state
 * @param[in] base what to search for (nullptr == "KMD")
 * @returns the correct vaporum_state object
 */
vaporum_state *vaporum_stateptrget(char *base)
{
    // "KMD" case
 ...
 * @returns the vaporum_state object for symbol
 */
vaporum_state *vaporum_stateptr(char *symbol,char *dest)
{
    int32_t baseid;
    vaporum_nameset(symbol,dest,chainName.symbol().c_str());
    return(vaporum_stateptrget(symbol));
}
...
bool vaporum_is_vSolutionsFixActive()
{
    return GetLatestTimestamp(vaporum_currentheight()) > nS5Timestamp;
}
