...
#define VAPORUM_ELECTION_GAP 2000    //((ASSETCHAINS_SYMBOL[0] == 0) ? 2000 : 100)
...
extern uint256 VAPORUM_EARLYTXID;
...
extern uint32_t VAPORUM_STOPAT; // set by -stopat, will not add more blocks after specified height
extern uint32_t VAPORUM_DPOWCONFS; // set by -dpowconfs, normally 0/1
...
extern uint64_t VAPORUM_INTERESTSUM; // calculated value, returned in getinfo() RPC call
extern uint64_t VAPORUM_WALLETBALANCE; // pwalletmain->GetBalance(), returned in getinfo() RPC call
...
extern std::mutex vaporum_mutex; // seems to protect PAX values and Pubkey array
//extern std::vector<uint8_t> Mineropret; // previous miner values
extern pthread_mutex_t VAPORUM_CC_mutex; // mutex to help with CryptoConditions
extern CScript VAPORUM_EARLYTXID_SCRIPTPUB; // used mainly in cc/prices.cpp
