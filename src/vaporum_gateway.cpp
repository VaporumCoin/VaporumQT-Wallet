...
#include "vaporum_utils.h" // vaporum_stateptrget
#include "vaporum_bitcoind.h" // vaporum_checkcommission
 ...
 */
bool vaporum_checkvout(int32_t vout,int32_t k,int32_t indallvouts)
{
 ...
 */
int32_t vaporum_bannedset(int32_t *indallvoutsp,uint256 *array,int32_t max)
{
    if ( sizeof(banned_txids)/sizeof(*banned_txids) > max )
    {
        LogPrintf("vaporum_bannedset: buffer too small %d vs %d\n",(int32_t)(sizeof(banned_txids)/sizeof(*banned_txids)),max);
        StartShutdown();
    }
    int32_t i;
 ...
 */
int32_t vaporum_check_deposit(int32_t height,const CBlock& block)
{
            ...
            for (int32_t j=0; j<n; j++) // for each vin
            {
                for (int32_t k=0; k<numbanned; k++) // for each banned txid
                {
                    if ( block.vtx[i].vin[j].prevout.hash == array[k] 
                            && vaporum_checkvout(block.vtx[i].vin[j].prevout.n,k,indallvouts) )
                    {
         ...
         NetworkUpgradeActive(height, Params().GetConsensus(), Consensus::UPGRADE_SAPLING) )
    {
        ...
        if ( chainName.isKMD() )
        {
            ...
            else if ( height > 814000 )
            {
                ...
                int32_t num;
                return(-1 * (vaporum_electednotary(&num,script+1,height,0) >= 0) * (height > 1000000));
            }
        }
        else
        {
            ...
            {
                if ( (checktoshis= vaporum_checkcommission((CBlock *)&block,height)) < 0 )
                {
...
void vaporum_stateind_set(struct vaporum_state *sp,uint32_t *inds,int32_t n,uint8_t *filedata,long datalen,const char *symbol,const char *dest)
{
        ...
        for (lastfpos=fpos=prevpos100=i=0; i<n; i++)
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    if ( doissue != 0 )
                    {
                        vaporum_parsestatefiledata(sp,filedata,&lastfpos,datalen,symbol,dest);
                        count++;
                    }
                }
            }
...
/**
 * @brief Validate the index of the vaporumstate file
 * 
 * @param[in] sp the vaporum_state struct
 ...
 * @return -1 on error
 */
long vaporum_stateind_validate(struct vaporum_state *sp,const std::string& indfname,uint8_t *filedata,long datalen,
    ...
    if ( (inds= OS_fileptr(&fsize,indfname.c_str())) != 0 )
    {
        ...
        if ( (fsize % sizeof(uint32_t)) == 0 )
        {
            ...
            *prevpos100p = prevpos100;
            if ( sp != 0 )
                vaporum_stateind_set(sp,(uint32_t *)inds,n,filedata,fpos,symbol,dest);
            free(inds);
            return fpos;
        } 
        else 
...
long vaporum_indfile_update(FILE *indfp,uint32_t *prevpos100p,long lastfpos,long newfpos,uint8_t func,uint32_t *indcounterp)
{
    if ( indfp != 0 )
    {
...
/***
 * @brief read the vaporumstate file
 * @param sp the vaporum_state struct
 * @param fname the filename
 * @param symbol the chain symbol
 * @param dest the "parent" chain
 * @return true on success
 */
bool vaporum_faststateinit(vaporum_state *sp,const char *fname,char *symbol, const char *dest)
{
    ...
    if ( (filedata= OS_fileptr(&datalen,fname)) != 0 )
    {
        ...
        int32_t func;
        while (!ShutdownRequested() && (func= vaporum_parsestatefiledata(sp,filedata,&fpos,datalen,symbol,dest)) >= 0)
        {
            lastfpos = vaporum_indfile_update(indfp,&prevpos100,lastfpos,fpos,func,&indcounter);
        }
        if (ShutdownRequested()) { fclose(indfp); return false; }
        if ( indfp != nullptr )
        {
            fclose(indfp);
            if ( (fpos= vaporum_stateind_validate(0,indfname,filedata,datalen,&prevpos100,&indcounter,symbol,dest)) < 0 )
                LogPrintf("unexpected vaporumstate.ind validate failure %s datalen.%ld\n",indfname.c_str(),datalen);
            else 
...
//uint64_t vaporum_interestsum(); // in wallet/rpcwallet.cpp

/***
 * @brief update wallet balance / interest
 * @note called only on KMD chain every 10 seconds ( see ThreadUpdateVaporumInternals() )
 */
void vaporum_update_interest()
{
    static uint32_t lastinterest; // prevent needless vaporum_interestsum calls
    if (vaporum_chainactive_timestamp() > lastinterest)
    {
        vaporum_interestsum();
        lastinterest = vaporum_chainactive_timestamp();
    }
