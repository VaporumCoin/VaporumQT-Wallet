...
#include "vaporum_curve25519.h" // for vaporum_kvsigverify
#include <mutex>
 ...
 */
uint256 vaporum_kvsig(uint8_t *buf,int32_t len,uint256 _privkey)
{
 ...
 */
int32_t vaporum_kvnumdays(uint32_t flags)
{
    int32_t numdays;
...
uint64_t vaporum_kvfee(uint32_t flags,int32_t opretlen,int32_t keylen)
{
    ...
    if ( (k= keylen) > 32 )
        k = 32;
    numdays = vaporum_kvnumdays(flags);
 ...
 */
int32_t vaporum_kvsearch(uint256 *pubkeyp, int32_t current_height, uint32_t *flagsp,
 ...
 */
void vaporum_kvupdate(uint8_t *opretbuf,int32_t opretlen,uint64_t value)
{
    static uint256 zeroes;
    ...
    if ( keylen+13 > opretlen )
    {
        ...
        if ( ++counter < 1 )
            LogPrintf("vaporum_kvupdate: keylen.%d + 13 > opretlen.%d, this can be ignored\n",keylen,opretlen);
        return;
    }
    uint8_t *valueptr = &key[keylen];
    uint64_t fee = vaporum_kvfee(flags,opretlen,keylen);
    if ( value >= fee )
    {
                ...
                || opretlen == coresize+2*sizeof(uint256) )
        {
            ...
            uint256 refpubkey;
            int32_t kvheight;
            int32_t refvaluesize = vaporum_kvsearch(&refpubkey,height,
            ...
            else if ( ptr == 0 )
            {
                ...
                ptr = (vaporum_kv *)calloc(1,sizeof(*ptr));
