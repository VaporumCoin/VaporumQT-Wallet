AC_DEFUN([VAPORUM_FIND_BDB62],[
  ...
  if test "x$bdbpath" = "xX"; then
    AC_MSG_RESULT([no])
    AC_MSG_ERROR([libdb_cxx headers missing, Vaporum Core requires this library for wallet functionality (--disable-wallet to disable wallet functionality)])
  elif test "x$bdb62path" = "xX"; then
    VAPORUM_SUBDIR_TO_INCLUDE(BDB_CPPFLAGS,[${bdbpath}],db_cxx)
    ...
    VAPORUM_SUBDIR_TO_INCLUDE(BDB_CPPFLAGS,[${bdb62path}],db_cxx)
    bdbpath="${bdb62path}"
  fi
  ...
  if test "x$BDB_LIBS" = "x"; then
      AC_MSG_ERROR([libdb_cxx missing, Vaporum Core requires this library for wallet functionality (--disable-wallet to disable wallet functionality)])
  fi
  AC_SUBST(BDB_LIBS)
])
